-- Persistent League Manager live-session state for the Next/Vercel migration.
-- Streamlit remains the operator fallback, but Next League Live can now resume
-- interrupted league nights and retain round/court audit context.

create extension if not exists pgcrypto;

create table if not exists league_live_sessions (
...
create index if not exists idx_league_live_sessions_club_status_updated
    on league_live_sessions (club_id, status, updated_at desc);

create index if not exists idx_league_live_sessions_club_league_updated
    on league_live_sessions (club_id, league_name, updated_at desc);

create table if not exists league_live_rounds (
    id uuid primary key default gen_random_uuid(),
    club_id text not null,
    session_id uuid not null references league_live_sessions(id) on delete cascade,
...
create index if not exists idx_league_live_rounds_club_session_round
    on league_live_rounds (club_id, session_id, round_number);

create table if not exists league_live_courts (
    id uuid primary key default gen_random_uuid(),
    club_id text not null,
    session_id uuid not null references league_live_sessions(id) on delete cascade,
    round_number integer not null,
...
create index if not exists idx_league_live_courts_club_session_round
    on league_live_courts (club_id, session_id, round_number, court_number);
