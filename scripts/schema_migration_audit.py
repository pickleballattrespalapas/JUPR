...
CREATE_SCHEMA_RE = re.compile(
    r"\bCREATE\s+SCHEMA\s+(?:IF\s+NOT\s+EXISTS\s+)?(?P<schema>[\w\"]+)",
    re.IGNORECASE,
)
...
RENAME_TO_RE = re.compile(r"\bRENAME\s+TO\s+(?P<newname>[\w\"]+)", re.IGNORECASE)
SET_SCHEMA_RE = re.compile(r"\bSET\s+SCHEMA\s+(?P<newschema>[\w\"]+)", re.IGNORECASE)
...
def inventory_anomalies(files: Iterable[Path]) -> tuple[dict[str, list[str]], list[str], list[str]]:
    """
    Returns:
      - duplicates: {timestamp: [filenames]}
      - missing_ts: [filenames]
      - non_monotonic: [filenames]  # timestamp <= previous timestamp in sorted order
    """
    ...
    for path in files:
        ...
        if prev_ts is not None and ts <= prev_ts:
            non_monotonic.append(path.name)
        prev_ts = ts

    duplicates = {k: v for k, v in duplicates.items() if len(v) > 1}
    return duplicates, missing_ts, non_monotonic


def _statement_slice(sql: str, start_idx: int) -> str:
    """
    Best-effort: return text from start_idx to the next ';' (or end of string).
    This avoids the old 250-char window for rename detection.
    """
    end = sql.find(";", start_idx)
    if end == -1:
        end = len(sql)
    return sql[start_idx:end]
...
def audit_sql_migrations(files: list[Path], initial_tables: set[str]) -> list[Violation]:
    ...
    for path in files:
        ...
        for m in ALTER_TABLE_RE.finditer(cleaned):
            ...
            stmt = _statement_slice(cleaned, m.start())

            # Handle ALTER TABLE ... RENAME TO ...
            rename_match = RENAME_TO_RE.search(stmt)
            if rename_match:
            ...
            set_schema_match = SET_SCHEMA_RE.search(stmt)
            if set_schema_match:
                newschema = set_schema_match.group("newschema").strip('"').lower()
                old_table_name = table.split(".", 1)[1]
                new_table = f"{newschema}.{old_table_name}"
...
def main() -> int:
    ...
    for source in sources:
        ...
        duplicates, missing_ts, non_monotonic = inventory_anomalies(source.files)
        ...
        if non_monotonic:
            print("  Non-monotonic timestamp order (timestamp <= previous file):")
