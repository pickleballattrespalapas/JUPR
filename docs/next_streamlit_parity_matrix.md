...
| `league_manager` | 🏟️ League Manager | Admin | `Partial` — Next `/admin/league-manager` has guarded draft creation and configuration-only duplication, explicit start/pause/resume/end/archive lifecycle actions, server-enforced draft/running/closed settings locks, list/detail, roster membership, schedule-preview ICS export, printout, persisted `/admin/league-manager/live` sessions with court movement, and `/admin/league-manager/awards` top-performer close/award workflow. | Yes | Admin auth + match-write + audit gate | Validate draft create/duplicate, lifecycle/settings locks, schedule ICS, live sessions, court movement, awards close, and recovery through Match Log/Replay History in staging. |
...
| `challenge_ladder_admin` | 🛠️ Challenge Ladder Admin | Admin | `Partial` — Next `/admin/challenge-ladder` has guarded dashboard, tier roster review, challenge buckets, and cancel/forfeit actions. | Yes | Admin auth + ladder write/audit gate | Validate with staging ladder data before adding full challenge creation/result/rank-movement parity. |
...
14. Validate Challenge Ladder Admin cancel/forfeit-only slice before adding challenge creation/result/rank movement.
