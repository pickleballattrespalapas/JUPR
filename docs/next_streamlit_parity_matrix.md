...
| `league_manager` | 🏟️ League Manager | Admin | `Partial` — Next `/admin/league-manager` has guarded list/detail, schedule-preview, config, standings, roster snapshot, settings editor, roster membership editor, printout, and `/admin/league-manager/live` one-round live score entry through Match Uploader. | Yes | Admin auth + match-write + audit gate | Validate live one-round entry in staging, then add persistent multi-round court movement/ladder state and awards. |
...
| `challenge_ladder_admin` | 🛠️ Challenge Ladder Admin | Admin | `Not started`; listed in `/admin` operations cockpit. | Yes | Admin auth + ladder write/audit gate | Keep lowest priority until core tournament, league, player update, and email workflows are proven. |
...
15. Validate League Manager live one-round entry and browser-print exports.
16. Validate Player Updates Admin date-range reports with `JUPR_EMAIL_MODE=dry_run` or `staging_redirect` before live email.
17. Validate Player Editor create/update, league-rating edits, social identity linking, and merge preview/execute with immediate Replay History recovery.
18. Validate League Manager settings and roster membership in staging.
19. Validate Tournament Admin/Ops setup/import/scoring/podium/award/official-publish workflows in staging.
20. Validate Tournament Live runner separately as the in-play tournament draw surface.
21. Validate admin login, password reset, sign-out, expired-token, wrong-club, and permission-denied cases in staging.
22. Port remaining persistent League Manager court movement state, Weekly Recap Admin, badge diagnostics, verified update requests, Moneyball, and Challenge Ladder Admin after the current write foundation is proven.
