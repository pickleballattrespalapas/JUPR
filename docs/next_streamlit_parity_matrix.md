...
| `match_log` | 📝 Match Log | Admin | `Partial` — Next Match Log read, duplicate scan, guarded apply, duplicate cleanup, and Replay History foundation exist and use the stored Next admin Supabase session for guarded writes. | Yes | Admin auth + replay/correction audit gate | Validate closed-club pilot; add replay job history/approval if needed. |
...
| `admin_login` | 🔐 Admin Login | Hidden/auth | `Partial` — Next `/admin/login` Supabase Auth shell exists, `/admin` shows the browser admin session, and guarded write forms consume the stored session token. | No | Supabase Auth/session + auth E2E gate | Add staging auth E2E for sign-in/sign-out, expired token, wrong-club, and permission-denied cases before broad admin cutover. |
