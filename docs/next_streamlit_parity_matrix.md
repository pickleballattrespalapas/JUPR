...
| `admin_tools` | ⚙️ Admin Tools | Admin | `Partial` — Replay History moved to `/admin/replay-history`; Badge Diagnostics, Match Canonical Audit, activity/role administration, badge queue status/processing, scoped badge recompute with dry-run, and a read-only missing tournament-match backfill preview use stored Next admin sessions and Python/FastAPI services. Tournament backfill apply and remaining maintenance migrations stay Streamlit-only. | Yes/high-risk | Super-admin-only + worker/backfill gate | Validate badge worker/recompute in isolated staging; keep tournament backfill apply and migrations on Streamlit until a reviewed apply contract and recovery runbook exist. |
...
16. Validate badge queue/recompute status and dry-run behavior, then port tournament-match backfill apply and remaining super-admin migrations only after reviewed apply contracts and recovery runbooks are hardened.
