...
| `badge_codex` | 📼 Badge Codex | Public | `Partial` — public FastAPI badge/earner APIs and Next `/clubs/[clubSlug]/badge-codex` route exist with section filters, load-more links, direct badge anchors, and recent-earner trophy room. | No | Public smoke + contract tests | Add authoritative status/timing/scope buckets and per-badge earner pagination, then validate every bucket, anchor, empty/error state, and trophy-room layout. |
