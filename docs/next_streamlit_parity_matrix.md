...
| `league_manager` | 🏟️ League Manager | Admin | `Partial` — Next `/admin/league-manager` has guarded draft creation and configuration-only duplication, explicit start/pause/resume/end/archive lifecycle actions, server-enforced draft/running/closed settings locks, list/detail, roster membership, printout, persisted `/admin/league-manager/live` sessions with court movement, and `/admin/league-manager/awards` top-performer close/award workflow. | Yes | Admin auth + match-write + audit gate | Validate draft create/duplicate, lifecycle/settings locks, live sessions, court movement, awards close, and recovery through Match Log/Replay History in staging. |
