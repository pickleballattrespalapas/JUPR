...
| `league_manager` | 🏟️ League Manager | Admin | `Partial` — Next `/admin/league-manager` has guarded list/detail, schedule-preview, config, standings, roster snapshot, settings editor, roster membership editor, printout, persisted `/admin/league-manager/live` sessions with court movement, and `/admin/league-manager/awards` top-performer close/award workflow. | Yes | Admin auth + match-write + audit gate | Validate live sessions, court movement, awards close, and recovery through Match Log/Replay History in staging. |
...
15. Validate League Manager live sessions, court movement, browser-print exports, and awards close workflow.
...
22. Port remaining Weekly Recap Admin, badge diagnostics, verified update requests, Moneyball, and Challenge Ladder Admin after the current write foundation is proven.
