...
| `league_manager` | 🏟️ League Manager | Admin | `Partial` — Next `/admin/league-manager` has guarded draft creation, list/detail, description/settings edits, roster membership, printout, persisted `/admin/league-manager/live` sessions with court movement, and `/admin/league-manager/awards` top-performer close/award workflow. | Yes | Admin auth + match-write + audit gate | Validate draft creation, live sessions, court movement, awards close, and recovery through Match Log/Replay History in staging. |
