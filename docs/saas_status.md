...
- The current live staging readback has 38 migration-ledger entries through the
  connector-assigned `20260725193213_singles_replay_recovery` head. The repair
  candidate introduces the reviewed `challenge_ladder_public_results`
  migration, making the required inventory 39 logical names; its
  connector-assigned staging ledger head remains pending until application and
  readback. All 29 implementation orders have landed,
...
| Migrations | In progress | `supabase/migrations/` is canonical; the repaired repository profile contains 39 logical names including managed singles replay recovery and Challenge Ladder public-result relations, while the current staging readback contains 38. | Apply the new Challenge Ladder migration to staging only, record the connector ledger version/head and schema/RPC probes, then keep staging-first discipline before any separately approved production SQL. | Medium |
