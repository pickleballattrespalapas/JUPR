...
| CI/API contract tests | In progress | `make api-test` covers API contracts; `scripts/smoke_public_web.py` covers staged public API/web smoke checks. | Wire public smoke into GitHub Actions secrets/inputs and require it before public read-only cutover. | Medium |
