...
Order 24 adds a stricter staging-only durability envelope around Challenge Ladder, Moneyball, and one-off JUPR Live. Every mutation requires the current Python version or reviewed settlement/result fingerprint, a stable idempotency key, a private service-role operation ledger, required intent/completion/failure audits, and a concrete Match Log/Replay recovery handoff. Exact retries return the stored result before recomputing a changed preview. JUPR Live official publish claims the session version before submitting matches so an interrupted batch remains visible and cannot be blindly republished. See `docs/live_ladder_parity_runbook.md`; these surfaces remain on Streamlit fallback outside staging and remain `Partial` until manual acceptance.

Admin Tools now exposes authenticated activity/role administration, club-scoped read-only overall/league rating reports with browser-generated CSV export, Club Social submission moderation, badge queue status/processing, scoped badge recompute with dry-run, guarded badge-definition lifecycle controls under `/admin/badges`, and a reviewed tournament-match backfill workflow. Rating reports preserve Streamlit JUPR, win percentage, and gain calculations while excluding inactive/merged players from OVERALL. Club Social review lists only the selected club's unrated submissions and requires `manage_matches`, an exact confirmation phrase, a current expected status, and a reason for rejection before changing moderation status; each moderation is recorded in centralized Admin Tools activity. Badge-definition state changes use the Python `live → frozen → deprecated` state machine and require `manage_roles`, a reason, exact confirmation, expected-state locking, and explicit force for a nonstandard transition. The tournament preview identifies finalized games missing a club-scoped official match and classifies ready versus blocked candidates without writing rows. Apply accepts only selected ready IDs from the current fingerprinted preview, caps each batch, rechecks club players and duplicates, writes audit intent/completion records, verifies exact persisted IDs, and routes failures to Match Log plus Replay History recovery. Remaining one-time maintenance migrations stay on Streamlit fallback until reviewed dry-run/apply/recovery contracts are added.
