...
|---|---:|---|
| Unsafe-method FastAPI contracts | **199** | Complete AST inventory |
| `POST` / `PATCH` / `PUT` / `DELETE` | **160 / 30 / 8 / 1** | All enumerated below |
...
| Contracts in `OPEN_WRITE_ROUTES` | **199** | Exact match to the AST inventory |
...
| `BE-199` | **Implemented** | Post-publish movement requires official persisted scores, a server-recomputed Python plan, the reviewed plan key, current session version, and `APPLY LEAGUE MOVEMENT`. Exact replay returns the already-applied movement without republishing scores. | Session and round updates use CAS plus compensating rollback rather than one SQL RPC; a dedicated atomic movement RPC remains the preferred final form. |
...
The inventory was generated from Python AST traversal of all route modules beneath `services/api`, then statically traced through request models and imported `jupr_app/services` functions. The original filename/line traversal IDs `BE-001` through `BE-192` are preserved unchanged. Five registered reconciliation contracts are appended as `BE-193` through `BE-197`, followed by the retained League Live publish retry as `BE-198` and score-first movement apply as `BE-199`; future routes must continue appending rather than renumbering existing rows.
...
| League Live submit/recovery: `BE-030`–`BE-033`, `BE-196`, `BE-198`–`BE-199` | `jupr_app/services/admin_league_live_submit_service.py`, `admin_league_live_service.py`; live-operation tables/RPCs in Supabase migrations | Expected session state plus idempotency for score publish/create; explicit operation keys on score context, movement apply, and session-create reconciliation; zero-evidence retry reuses the server-retained request/key | Score submit, movement apply, retained round retry, round reconcile, verified compensation, and proof-only session-create reconcile are distinct guarded actions | Strong; movement apply uses CAS plus compensating rollback rather than one RPC |
...
| `BE-025`–`BE-033`, `BE-196`, `BE-198`–`BE-199` | League Manager live rounds, retained publish retry, score-first movement apply, and session-create recovery | `apps/web/app/admin/league-manager/live/LeagueLiveRoundPanel.tsx` |
...
These IDs are deliberately appended after the original `BE-001`–`BE-192` inventory. `BE-193`–`BE-197` inspect frozen operation evidence and authoritative stored state without rerunning the original mutation. `BE-198` is a distinct zero-evidence retry: it replays only the exact server-retained League Live request and idempotency key after verifying that the durable state is retryable. `BE-199` applies a reviewed movement plan only after the official score publish is complete.
...
| BE-199 | League Live — `POST /admin/clubs/{club_id}/league-manager/live-sessions/{session_id}/rounds/{round_number}/movement` — `post_admin_league_live_round_movement` | `APPLY LEAGUE MOVEMENT` | expected_updated_at,expected_operation_key,persisted official scores | expected_operation_key | league-live-submit |
...
```text
unsafe_routes=199 manifest=199 missing=0 stale=0 overlaps=12 open_flags=32
```

The repository's boundary-equivalent tests are:

```bash
# Registered unsafe-method contracts (AST-based in the audit script): 199
# OPEN_WRITE_ROUTES entries: 199
...
Re-run the AST/manifest comparison whenever a FastAPI route decorator changes. The stable-ID table should be updated in the same pull request; new unsafe routes receive IDs after `BE-199`. Four authenticated operation-inspection routes remain safe-method `GET` contracts; five separately appended proof-only reconciliation routes `BE-193`–`BE-197`, the server-retained League Live retry `BE-198`, and score-first movement apply `BE-199` are `POST` contracts, raising the unsafe-route and staging-manifest totals from 192 to 199.
