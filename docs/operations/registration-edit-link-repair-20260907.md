...
Joe approved configuring the missing key and deploying this repair on
7 September 2026. The production release workflow now checks signing readiness
after SMTP authentication and before its existing runtime projection. Only a
release trigger or protected manual dispatch explicitly carrying the boolean
`initialize_registration_edit_secret=true` permits missing-key initialization.
Omitting it performs a read-only check and cannot generate or rotate a key.
The existing exact-candidate, trigger-only commit, rollback identity, migration,
runtime and CORS checks remain enforced. Both production API origins must report
edit signing configured before the release is accepted.
