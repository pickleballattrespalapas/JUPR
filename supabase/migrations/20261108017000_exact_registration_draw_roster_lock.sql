-- Scope registration edit locks to registrations that were actually rostered.
-- An imported team elsewhere in the same division must not freeze excluded
-- NEEDS_PARTNER or otherwise unresolved registrations. Draw scope is resolved
-- from the authoritative draw row before the team's denormalized scope.
...
begin
  if to_regprocedure(
    'public.server_update_public_tournament_registration_edit(text,text,timestamp with time zone,jsonb,jsonb,jsonb)'
     ...
     or to_regclass('public.tournament_teams') is null then
    raise exception using
      errcode = '42P01',
      message = 'Tournament draw and public registration edit guards must exist before exact roster locking.';
  end if;
  ...
  if not exists (
    ...
    where table_schema = 'public'
      ...
      and column_name = 'source_selection_id'
  ) then
    raise exception using
      errcode = '42703',
      message = 'Exact tournament registration roster provenance is unavailable.';
  end if;
end
$migration_preflight$;
...
begin
  ...
  if exists (
    ...
    join public.tournament_teams as team
     ...
     and upper(coalesce(team.source, '')) in (
       'REGISTRATION',
       'REGISTRATION_COMBINED_RATING'
     )
     and (
       team.source_selection_id::text = selection.id
       or (
         v_registration.player_id is not null
         and v_registration.player_id in (team.player1_id, team.player2_id)
       )
     )
    left join public.tournament_event_draws as draw
      on draw.id = team.draw_id
     and draw.tournament_id = team.tournament_id
    where selection.tournament_id = btrim(p_tournament_id)
      and selection.registration_id = btrim(p_registration_id)
      and coalesce(
        draw.registration_day_id::text,
        team.registration_day_id::text
      ) = selection.registration_day_id
      and coalesce(
        draw.event_option_id::text,
        team.event_option_id::text
