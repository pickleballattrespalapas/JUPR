-- Admit the durable League Live match context to the atomic direct-match
-- transaction. The League Live publisher has always generated a unique UUIDv5
-- context_id for every session/round/match slot, and the reconciliation schema
-- already enforces its club-scoped uniqueness. The original atomic validator
-- predated that context and accepted only blank/event rows.
--
-- Replace the private compatibility implementation directly so the real
-- league_live_session context is validated and inserted in the same
-- transaction. Do not transiently disguise League Live rows as event rows.

do $migration$
begin
  if pg_catalog.to_regprocedure(
    'public.admin_apply_direct_match_entry_atomic_v1_base_20260727(uuid,text,text,text,text,text,text,text,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb)'
  ) is null then
    raise exception using
      errcode = '42883',
      message = 'League Live direct-match context support requires the atomic direct-match base RPC';
  end if;
end
$migration$;

create or replace function public.admin_apply_direct_match_entry_atomic_v1_base_20260727(
  p_operation_id uuid,
  p_club_id text,
...
begin
     ...
     where match_row.club_id is distinct from v_club_id
        ...
        or (
          ...
          and (
            ...
            or pg_catalog.btrim(match_row.context_id) !~*
  ...
  with inserted as (
    ...
    select
      ...
      nullif(pg_catalog.btrim(match_row.context_type), ''),
      nullif(pg_catalog.btrim(match_row.context_id), ''),
...
comment on function public.admin_apply_direct_match_entry_atomic_v1_base_20260727(
  uuid,
  text,
  text,
  text,
  ...
  'Private atomic direct-match implementation; accepts durable event and League Live contexts for service-role callers.';
