...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Orphan tournament_teams rows detected (missing parent tournaments)';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    LEFT JOIN public.tournaments t ON t.id = tg.tournament_id
    WHERE t.id IS NULL
  ) THEN
    RAISE EXCEPTION 'Orphan tournament_games rows detected (missing parent tournaments)';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM public.tournament_podium tp
    LEFT JOIN public.tournaments t ON t.id = tp.tournament_id
    WHERE t.id IS NULL
  ) THEN
    RAISE EXCEPTION 'Orphan tournament_podium rows detected (missing parent tournaments)';
  END IF;

  IF EXISTS (
    SELECT 1
    FROM public.tournament_podium tp
    LEFT JOIN public.tournament_teams tt ON tt.id = tp.team_id
    WHERE tt.id IS NULL
  ) THEN
    RAISE EXCEPTION 'Orphan tournament_podium rows detected (missing referenced tournament_teams)';
  END IF;
END $$;
