...
def test_repository_migration_inventory_and_reviewed_profile_are_deterministic() -> None:
    ...
    assert {"20260906004056", "20260906005541", "20260906052216", "20260908052027"}.issubset(versions)
    assert versions[-34:] == (
        "20261030010000",
        "20261101000000",
        "20261102000000",
    ...
    assert len(contract["required_ledger_names"]) == 105
    assert {"club_staff_scopes", "pcs_platform_onboarding", "club_admin_settings"}.issubset(contract["required_ledger_names"])
