...
def test_repository_migration_inventory_and_reviewed_profile_are_deterministic() -> None:
    ...
    assert len(contract["required_ledger_names"]) == 110
    assert {"club_staff_scopes", "pcs_platform_onboarding", "club_admin_settings", "staff_invitations", "staff_invitation_verified_identity", "interclub_participation_rosters", "interclub_meet_rosters", "interclub_setup_wizard"}.issubset(contract["required_ledger_names"])
