...
def test_repository_migration_inventory_and_reviewed_profile_are_deterministic() -> None:
    ...
    assert len(versions) == 96
    assert {"20260906004056", "20260906005541"}.issubset(versions)
    assert versions[-28:] == (
        "20261030010000",
        "20261101000000",
        "20261102000000",
    ...
    assert len(contract["required_ledger_names"]) == 96
...
def test_preflight_accepts_only_matching_protected_project_and_config() -> None:
    ...
    assert errors == []
    assert migrations[-1] == "20261108028000"
