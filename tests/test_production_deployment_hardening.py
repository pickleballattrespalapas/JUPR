...
def test_repository_migration_inventory_and_reviewed_profile_are_deterministic() -> None:
    ...
    assert len(versions) == 139
    assert "20260923215248" in versions
    assert "interclub_meet_signup_gender" in names
    assert {"20260923201534", "20260923203433"}.issubset(versions)
    assert {"interclub_meet_signup_queues", "interclub_meet_signup_lookup_index"}.issubset(contract["required_ledger_names"])
    assert "20260923003009" in versions
    assert "tournament_gender_eligibility_reviews" in contract["required_ledger_names"]
    assert "20260922174500" in versions
    assert "interclub_inline_player_registration" in contract["required_ledger_names"]
    assert {"20260922161246", "20260922161655"}.issubset(versions)
    assert {"interclub_late_player_requests", "interclub_meet_schedule_adjustments"}.issubset(contract["required_ledger_names"])
    ...
    assert "20260921170349" in versions
    assert "20260921180113" in versions
    ...
    assert len(contract["required_ledger_names"]) == 139
