...
def test_snapshot_write_gate_uses_the_production_aware_tournament_live_surface(
    monkeypatch,
) -> None:
    monkeypatch.setenv("JUPR_ENABLE_NEXT_ADMIN_TOURNAMENTS", "1")
    monkeypatch.setenv(
    ...
    monkeypatch.setenv("JUPR_ENABLE_TOURNAMENT_WRITES_PRODUCTION", "1")

    def writes_enabled() -> bool:
        return bool(
            build_admin_tournament_day_live_snapshot(
                FakeSupabase(snapshot_tables()),
                club_id="club-1",
                tournament_id="tour-1",
                registration_day_id="day-1",
    ...
    monkeypatch.delenv("JUPR_PRODUCTION_WRITE_POLICY")
    assert writes_enabled() is False
    monkeypatch.setenv("JUPR_PRODUCTION_WRITE_POLICY", "enabled")

    monkeypatch.delenv("JUPR_ENABLE_STAGING_NEXT_ADMIN_TOURNAMENT_LIVE_WRITES")
    assert writes_enabled() is False
    monkeypatch.setenv(
        "JUPR_ENABLE_STAGING_NEXT_ADMIN_TOURNAMENT_LIVE_WRITES", "1"
    )
    ...
    monkeypatch.setenv("JUPR_ENV", "development")
    monkeypatch.setenv("JUPR_PRODUCTION_WRITE_POLICY", "enabled")
    monkeypatch.setenv("JUPR_ENABLE_TOURNAMENT_WRITES_PRODUCTION", "1")
    assert writes_enabled() is False
