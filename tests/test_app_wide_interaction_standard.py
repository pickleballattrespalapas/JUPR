...
def test_app_wide_standard_and_complete_inventories_are_checked_in() -> None:
    ...
    assert backend_ids == [f"BE-{index:03d}" for index in range(1, 197)]
    assert frontend_count + len(backend_ids) == 428

    method_counts = Counter(re.search(r"`(POST|PATCH|PUT|DELETE) ", row).group(1) for row in backend_rows)
    assert method_counts == {"POST": 157, "PATCH": 30, "PUT": 8, "DELETE": 1}
    assert sum("/admin/" in row for row in backend_rows) == 165
    ...
    assert "| Total audited frontend + backend contracts | **428** |" in report
    assert "| Staging write-wave coverage | **196 / 196** |" in report
