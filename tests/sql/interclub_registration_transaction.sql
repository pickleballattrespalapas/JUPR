...
begin
 ...
 out:=public.pcs_save_interclub_meet_roster(actor,actor_email,home,sid,mid,2,tid,2,'Home Blue','3.5',array[ids[1],ids[2],ids[3],ids[5]]);
 if out->'roster'->>'status'<>'needs_exception' then raise exception 'Over-limit player not blocked'; end if;
 begin
  perform public.pcs_review_interclub_meet_roster(actor,actor_email,home,sid,mid,2,tid,3,true,'Host approves');
  raise exception 'Host approved eligibility';
 exception when insufficient_privilege then null; end;
 begin
  perform public.pcs_review_interclub_meet_roster(actor,actor_email,org,sid,next_mid,1,tid,3,true,'Wrong meet');
  raise exception 'Decision attached through wrong meet';
 exception when no_data_found then null; end;
 out:=public.pcs_review_interclub_meet_roster(actor,actor_email,org,sid,mid,2,tid,3,true,'Organizer approved for this roster');
 if out->'roster'->>'status'<>'exception_approved' then raise exception 'Organizer decision failed'; end if;
 out:=public.pcs_save_interclub_meet_roster(actor,actor_email,home,sid,mid,2,tid,3,'Home Blue','3.5',array[ids[1],ids[2],ids[3],ids[6]]);
 ...
 begin
  perform public.pcs_save_interclub_meet_roster(actor,actor_email,home,sid,mid,2,tid,4,'Home Blue','3.5',ids[1:4]);
  raise exception 'Revoked admin changed roster';
 exception when insufficient_privilege then null; end;
 update public.admin_role_assignments set revoked_at=null where club_id=home;
 out:=public.pcs_save_interclub_meet_roster(actor,actor_email,home,sid,mid,2,tid,4,'','',array[]::bigint[],true);
 ...
 if (select roster from public.pcs_interclub_roster_versions where team_id=next_team and revision=1) is distinct from first_roster
