...
function pageHref({ clubSlug, week, section, print }: { clubSlug: string; week?: string | null; section?: RecapSection | null; print?: boolean }): string {
  ...
  return (
    <>
      ...
      {showSection(activeSection, "spotlight") ? (
        <section id="spotlight" style={{ pageBreakInside: "avoid" }}>
      ...
      {showSection(activeSection, "around") ? (
        <section id="around" style={{ pageBreakInside: "avoid" }}>
      ...
      {showSection(activeSection, "tournaments") && recap.tournaments?.length ? (
        <section id="tournaments" style={{ pageBreakInside: "avoid" }}>
          <h2>Tournaments</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "1rem" }}>
      ...
      {showSection(activeSection, "ahead") && recap.looking_ahead?.length ? (
        <section id="ahead" style={{ pageBreakInside: "avoid" }}>
          <h2>Looking Ahead</h2>
          <article style={cardStyle}>
            <ul style={{ margin: 0, paddingLeft: "1.25rem" }}>
        ...
        <div className="no-print" style={{ display: "grid", gap: "0.75rem", marginBottom: "1rem" }}>
          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
            {data.recaps.map((recap) => {
              ...
              return (
                <Link key={recap.week_start} href={pageHref({ clubSlug, week: recap.week_start, section: activeSection })} style={{ border: "1px solid #cbd5e1", borderRadius: "999px", padding: "0.45rem 0.75rem", background: active ? "#dbeafe" : "white", color: "#0f172a", textDecoration: "none", fontWeight: active ? 800 : 600 }}>
          ...
          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", alignItems: "center" }}>
            {(Object.keys(sectionLabels) as RecapSection[]).map((section) => {
              ...
              return (
                <Link key={section} href={pageHref({ clubSlug, week: selected?.week_start, section })} style={{ border: "1px solid #cbd5e1", borderRadius: "999px", padding: "0.45rem 0.75rem", background: active ? "#dcfce7" : "white", color: "#0f172a", textDecoration: "none", fontWeight: active ? 800 : 600 }}>
                  {sectionLabels[section]}
                </Link>
              );
            })}
            <Link href={pageHref({ clubSlug, week: selected?.week_start, section: activeSection, print: true })} style={{ border: "1px solid #cbd5e1", borderRadius: "999px", padding: "0.45rem 0.75rem", background: printMode ? "#fef9c3" : "white", color: "#0f172a", textDecoration: "none", fontWeight: 800 }}>
              Print view
            </Link>
            <PrintButton />
          </div>
            ...
            <div style={{ ...cardStyle, marginBottom: "1rem" }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: "0.75rem", flexWrap: "wrap" }}>
                ...
                <div className="no-print" style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                  ...
                  <a href={pageHref({ clubSlug, week: selected.week_start, section: activeSection, print: true })}>Print-friendly view</a>
                </div>
              </div>
            </div>
          ) : (
