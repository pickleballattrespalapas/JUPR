...
export default function GeneratorWorkspace({
  ...
  function requestBody(): Record<string, unknown> {
    ...
    if (hasAnyId && rawIds.some((value) => !value)) {
      throw new Error("Either link every entered player to a club player ID or leave all links blank.");
    }
    return {
      generator_kind: generatorKind,
      play_format: playFormat,
      ...
      participant_player_ids: hasAnyId ? Object.fromEntries(cleanRows.map((row, index) => [row.name.trim(), Number(rawIds[index])])) : {},
          ...
          {players.map((row, index) => (
            ...
            >
              ...
              <input
                ...
                placeholder="Club player ID optional"
                inputMode="numeric"
