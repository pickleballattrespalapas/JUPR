    ...
    <form onSubmit={onSubmit} style={{ display: "grid", gap: "1rem" }}>
      ...
      <section style={cardStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "0.75rem" }}>
          ...
          <label>Doubles skill<br /><input name="doubles_skill" type="number" min="1" max="7" step="0.01" style={{ width: "100%" }} /></label>
          <label>Singles skill<br /><input name="singles_skill" type="number" min="1" max="7" step="0.01" style={{ width: "100%" }} /></label>
          <label>Age<br /><input name="age" type="number" min="1" max="120" style={{ width: "100%" }} /></label>
          <label>Gender<br /><select name="gender" style={{ width: "100%" }}><option value="">Select</option><option>Women</option><option>Men</option><option>Non-binary</option><option>Prefer not to say</option></select></label>
        </div>
      </section>
                ...
                return (
                  <article key={eventOption.id} style={{ border: "1px solid #e2e8f0", borderRadius: "12px", padding: "0.75rem", background: selected ? "#f8fafc" : "white" }}>
                    <label style={{ display: "flex", gap: "0.5rem", alignItems: "flex-start" }}>
                      <input type="checkbox" checked={selected} onChange={(e) => toggleEvent(eventOption.id, e.target.checked)} />
                      ...
                      <div style={{ marginTop: "0.75rem", display: "grid", gap: "0.5rem" }}>
                        ...
                        {mode === "HAS_PARTNER" ? (
                          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "0.5rem" }}>
                            <label>Partner name<br /><input name={`partner_name_${eventOption.id}`} style={{ width: "100%" }} /></label>
                            <label>Partner email<br /><input name={`partner_email_${eventOption.id}`} type="email" style={{ width: "100%" }} /></label>
                        ...
                        {mode === "NEEDS_PARTNER" ? (
                          <label style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                            <input name={`show_on_partner_board_${eventOption.id}`} type="checkbox" /> Show me on the public partner board for this event
                          </label>
                        ) : null}
