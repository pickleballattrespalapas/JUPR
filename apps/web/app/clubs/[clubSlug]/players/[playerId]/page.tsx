...
function relationshipCard(clubSlug: string, title: string, testId: string, relationship?: PublicRelationship | null) {
  return (
    <article style={cardStyle} data-testid={testId}>
      ...
      ) : <p style={{ color: "#475569" }}>Not enough match history yet.</p>}
    </article>
  );
}
...
function RatingTrend({ points, clubSlug }: { points: PublicRatingHistoryPoint[]; clubSlug: string }) {
  ...
  if (!known.length) return <p data-testid="player-rating-trend-empty" style={{ color: "#475569" }}>No rating history is available yet.</p>;
...
export default async function PlayerProfilePage({ params, searchParams }: PlayerProfilePageProps) {
  ...
  if (error || !player || !data) {
    return (
      <section data-testid="player-profile-error-state">
        ...
        <p style={{ color: "#b91c1c" }}>We couldn&apos;t load this player profile. Please try again shortly.</p>
  ...
  const verifiedLabel = data.verified_updates.status === "enabled" ? "Player updates are on" : data.verified_updates.status === "pending" ? "Player update request is pending" : "Player updates are available";
  const verifiedAction = data.verified_updates.can_request
    ? <Link href={`/clubs/${clubSlug}/verified-updates?player_id=${encodeURIComponent(String(player.id))}`}>Request player updates</Link>
    : data.verified_updates.status === "pending"
      ? <span>Club staff will review this request.</span>
      : <span>Use the preferences link in a player update email to manage notifications.</span>;
  ...
  return (
    <section data-testid="player-profile">
      ...
      <p style={{ color: "#475569", maxWidth: "820px" }}>View this player&apos;s ratings, match history, awards, frequent partners and opponents, and Club Social results.</p>
      ...
      <article data-testid="player-public-identity" style={{ ...cardStyle, background: "#eff6ff", marginBottom: "1rem" }}>
        ...
        <p style={{ margin: "0.35rem 0", color: "#334155" }}>This profile shows the player&apos;s approved public name. Contact information and other private details are not shown.</p>
        {verifiedAction}
        ...
        <section data-testid="player-overview" style={{ display: "grid", gap: "1rem", marginBottom: "1rem" }}>
          <article style={cardStyle}>
            ...
            <p style={{ color: "#475569", marginBottom: 0 }}>
              Browse ratings, league positions, major honors, Club Social results, match history, and badges without leaving this player&apos;s profile.
            </p>
          </article>
      ...
      {sectionVisible(section, "positions") ? (
        <section id="positions" data-testid="player-league-positions" style={{ display: "grid", gap: "1rem", marginBottom: "1rem" }}>
          <article style={cardStyle}>
            ...
            <p style={{ color: "#475569", marginBottom: 0 }}>See this player&apos;s league rating, record, award standing, and eligibility.</p>
          </article>
          {leaguePositions.length === 0 ? <article style={cardStyle}>No league positions yet.</article> : leaguePositions.map((position) => {
            ...
            return (
              <article key={position.leagueName} style={cardStyle} data-testid="player-league-position">
                ...
                {position.error || !summary ? <p style={{ color: "#92400e" }}>We can’t show this player’s league position right now.</p> : <>
                  ...
                  <div style={{ borderTop: "1px solid #e2e8f0", paddingTop: "0.75rem" }}>
                    <strong>Award standings</strong>
                    {position.awards.length ? <ul style={{ marginBottom: 0 }}>{position.awards.map((award) => <li key={`${award.category_key}-${award.rank}`}>{award.category_label} — #{award.rank ?? "—"}{award.eligible_count ? ` of ${award.eligible_count} eligible` : ""} · {award.metric_display || "—"}{award.is_co_winner ? " · tied" : ""}</li>)}</ul> : <p style={{ color: "#475569", marginBottom: 0 }}>{eligible ? "No award standing is available yet." : `Awards become eligible after ${gameLabel(minimumGames)}.`}</p>}
                  </div>
                </>}
              </article>
            );
        ...
        <section id="ratings" data-testid="player-ratings" style={{ display: "grid", gap: "1rem", marginBottom: "1rem" }}>
          <article style={cardStyle}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "0.65rem", marginBottom: "1rem" }}>
              ...
              <div><strong>Change over last 10</strong><br />{signedRating(data.rating_summary.last_10_delta_jupr)}</div>
          ...
          <article style={cardStyle}>
            ...
            <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", minWidth: "600px" }}><thead><tr><th style={thStyle}>Format</th><th style={thStyle}>Matches</th><th style={thStyle}>Record</th><th style={thStyle}>Win %</th><th style={thStyle}>Rating change</th></tr></thead><tbody>
      ...
      {sectionVisible(section, "social") ? (
        <section id="social" data-testid="player-social" style={{ ...cardStyle, marginBottom: "1rem" }}>
          ...
          {!social.available ? <p style={{ color: "#92400e" }}>Club Social results are unavailable right now.</p> : null}
      ...
      {sectionVisible(section, "matches") ? (
        <section id="matches" data-testid="player-match-history" style={cardStyle}>
          ...
          {matches.length === 0 ? <p data-testid="player-match-empty" style={{ color: "#475569" }}>No matches match this view.</p> : <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", minWidth: "1040px" }}><thead><tr><th style={thStyle}>Date</th><th style={thStyle}>Format</th><th style={thStyle}>Result</th><th style={thStyle}>Team 1</th><th style={thStyle}>Score</th><th style={thStyle}>Team 2</th><th style={thStyle}>JUPR before → after</th><th style={thStyle}>League</th></tr></thead><tbody>
