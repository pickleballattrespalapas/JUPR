...
import editor from "../website/website.module.css";
import LeaderboardSettingsEditor from "../website/LeaderboardSettingsEditor";
...
function SettingsEditor({
  ...
  return <section>
    ...
    {site && doc ? <>
      <div className={`${editor.toolbar} ${editor.stickyToolbar}`} role="group" aria-label="Leaderboard settings actions">
        <div>
          <strong>{dirty ? "Unsaved changes" : "Draft saved"}</strong>
          <p className={editor.saveHint} role="status">{busy ? "Saving…" : message || "Save your draft, then publish to update the live leaderboard."}</p>
        </div>
        <div className={styles.actions}>
          <button type="button" className={dirty ? styles.primary : styles.button} disabled={busy || blocked || !dirty} onClick={() => void mutate("save")}>Save draft</button>
          <button type="button" className={dirty ? styles.button : styles.primary} disabled={busy || blocked || dirty} onClick={() => void mutate("publish")}>Publish settings</button>
        </div>
      </div>
      ...
      <div className={styles.actions} style={{ marginTop: "1rem" }}>
        <button type="button" className={styles.button} disabled={busy || blocked} onClick={() => void mutate("discard")}>Restore published settings</button>
      </div>
    </> : null}
  </section>;
}
