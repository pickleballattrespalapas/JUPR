...
async function loadStatus(clubId: string): Promise<{ data: StatusResponse | null; error: string | null }> {
  ...
  try {
    const response = await fetch(`${base.replace(/\/$/, "")}/admin/clubs/${encodeURIComponent(clubId)}/match-canonical-audit/status`, { next: { revalidate: 30 } });
...
export default async function MatchCanonicalAuditPage() {
  ...
  return (
    <section>
      ...
      <p style={{ color: "#334155", maxWidth: "880px" }}>Compare profile-visible matches against canonical player-match facts, inspect excluded legacy rows, and run explicit legacy normalization through FastAPI/Python domain services.</p>
