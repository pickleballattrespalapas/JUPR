...
function messageColor(message: string | null): string {
  ...
  if (text.includes("unable") || text.includes("error") || text.includes("disabled") || text.includes("missing")) return "#b91c1c";
  return "#166534";
}
...
export default function BadgeDiagnosticsPanel({ apiBase, clubId, status }: Props) {
  ...
  async function loadOptions() {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "Unable to load badge diagnostic options.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function runDebug() {
    if (!playerId || !badgeId) {
      setMessage("Choose a player and badge before running Badge Debug.");
      return;
    }
    setBusy(true);
    setMessage(null);
    try {
      ...
      setMessage(error instanceof Error ? error.message : "Unable to run Badge Debug.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function runAudit() {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "Unable to run Badge Audit.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div style={{ display: "grid", gap: "1rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "0.75rem", alignItems: "end" }}>
          <label>Player<br />
            <select value={playerId} onChange={(event) => setPlayerId(event.target.value)} style={inputStyle}>
              <option value="">All players for audit</option>
              {players.map((player) => <option key={player.id} value={player.id}>{player.name} #{player.id}</option>)}
            </select>
          </label>
          <label>Badge<br />
            <select value={badgeId} onChange={(event) => setBadgeId(event.target.value)} style={inputStyle}>
              <option value="">All badges for audit</option>
              {badges.map((badge) => <option key={badge.badge_id} value={badge.badge_id}>{badge.name} · {badge.badge_id}</option>)}
            </select>
          </label>
        ...
        {debugReport ? (
          <div style={{ marginTop: "1rem", display: "grid", gap: "0.75rem" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "0.75rem" }}>
              <article style={cardStyle}><strong>Raw matches</strong><br />{Array.isArray(debugReport.matches_raw) ? debugReport.matches_raw.length : "—"}</article>
              <article style={cardStyle}><strong>Filtered matches</strong><br />{Array.isArray(debugReport.matches_filtered) ? debugReport.matches_filtered.length : "—"}</article>
              <article style={cardStyle}><strong>Candidates</strong><br />{Array.isArray(debugReport.candidates) ? debugReport.candidates.length : "—"}</article>
              <article style={cardStyle}><strong>Errors</strong><br />{Array.isArray(debugReport.errors) ? debugReport.errors.length : "—"}</article>
            </div>
            {reportRows(debugReport, "candidates").length ? <Table title="Evaluator candidates" rows={reportRows(debugReport, "candidates").slice(0, 10)} /> : null}
            {reportRows(debugReport, "filter_audit_steps").length ? <Table title="Filter audit steps" rows={reportRows(debugReport, "filter_audit_steps").slice(0, 10)} /> : null}
            <details><summary>Diagnostics detail</summary><pre style={{ whiteSpace: "pre-wrap" }}>{JSON.stringify(debugReport.diagnostics || {}, null, 2)}</pre></details>
          </div>
        ) : null}
      </article>
      ...
      <article style={cardStyle}>
        ...
        <p style={{ color: "#475569" }}>Read-only expected-vs-actual badge audit. Use the scope above to narrow to a player, badge, league, date range, or context.</p>
        ...
        {auditReport ? (
          <div style={{ marginTop: "1rem", display: "grid", gap: "0.75rem" }}>
            ...
            {reportRows(auditReport, "missing_rows").length ? <Table title="Missing rows" rows={reportRows(auditReport, "missing_rows").slice(0, 20)} /> : null}
            {reportRows(auditReport, "stale_rows").length ? <Table title="Stale rows" rows={reportRows(auditReport, "stale_rows").slice(0, 20)} /> : null}
            {reportRows(auditReport, "duplicate_rows").length ? <Table title="Duplicate rows" rows={reportRows(auditReport, "duplicate_rows").slice(0, 20)} /> : null}
          </div>
        ) : null}
      </article>
    </div>
