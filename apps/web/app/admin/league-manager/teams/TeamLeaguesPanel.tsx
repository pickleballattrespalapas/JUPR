        ...
        <>
          ...
          {resultStatus === "complete" ? (
            <div style={{ ...gridStyle, marginTop: "0.75rem" }}>
              {[0, 1].map((index) => <label key={`a-${index}`}><strong>{teamName(detail, fixture.team_a_id)} player {index + 1}</strong><br /><SearchablePlayerSelect aria-label={[String(teamName(detail, fixture.team_a_id)),"player",String(index + 1)].join(" ")} value={playersA[index] || ""} onValueChange={playerValue => updateLineup("a", index, Number(playerValue))} style={inputStyle}>{optionsA.map((player) => <option key={player.id} value={player.id}>{player.name} · {player.sourceLabel}</option>)}</SearchablePlayerSelect></label>)}
              {[0, 1].map((index) => <label key={`b-${index}`}><strong>{teamName(detail, fixture.team_b_id)} player {index + 1}</strong><br /><SearchablePlayerSelect aria-label={[String(teamName(detail, fixture.team_b_id)),"player",String(index + 1)].join(" ")} value={playersB[index] || ""} onValueChange={playerValue => updateLineup("b", index, Number(playerValue))} style={inputStyle}>{optionsB.map((player) => <option key={player.id} value={player.id}>{player.name} · {player.sourceLabel}</option>)}</SearchablePlayerSelect></label>)}
            </div>
          ) : null}
        ...
        <>
          ...
          <article style={cardStyle}>
            ...
            <div style={gridStyle}>
              ...
              <label><strong>Captain</strong><br /><SearchablePlayerSelect aria-label="Captain" value={newCaptainId} onValueChange={playerValue => { setNewCaptainId(playerValue); if (newInitialPrimaryId === playerValue) setNewInitialPrimaryId(""); setCreateTeamKey(operationKey("team-create")); }} style={inputStyle}><option value="">Choose an active player</option>{detail.players.filter((player) => player.active !== false).map((player) => <option key={player.id} value={player.id}>{player.name}</option>)}</SearchablePlayerSelect></label>
              ...
              <label><strong>Initial primary (optional)</strong><br /><SearchablePlayerSelect aria-label="Initial primary (optional)" value={newInitialPrimaryId} onValueChange={playerValue => { setNewInitialPrimaryId(playerValue); setCreateTeamKey(operationKey("team-create")); }} style={inputStyle}><option value="">Add later</option>{detail.players.filter((player) => player.active !== false && String(player.id) !== newCaptainId).map((player) => <option key={player.id} value={player.id}>{player.name}</option>)}</SearchablePlayerSelect></label>
            ...
            <div style={gridStyle}>
              ...
              <label><strong>Player</strong><br /><SearchablePlayerSelect aria-label="Player" value={rosterPlayerId} onValueChange={playerValue => { setRosterPlayerId(playerValue); setRosterKey(operationKey("team-roster")); }} style={inputStyle}><option value="">Choose an active player</option>{detail.players.filter((player) => player.active !== false).map((player) => <option key={player.id} value={player.id}>{player.name}{player.gender ? ` · ${player.gender}` : ""}</option>)}</SearchablePlayerSelect></label>
            ...
            {detail.settings.substitute_pool_enabled ? <>
              <div style={gridStyle}>
                <label><strong>Player</strong><br /><SearchablePlayerSelect aria-label="Player" value={poolPlayerId} onValueChange={playerValue => { setPoolPlayerId(playerValue); setPoolKey(operationKey("team-substitute-pool")); }} style={inputStyle}><option value="">Choose an active player</option>{detail.players.filter((player) => player.active !== false).map((player) => <option key={player.id} value={player.id}>{player.name}</option>)}</SearchablePlayerSelect></label>
