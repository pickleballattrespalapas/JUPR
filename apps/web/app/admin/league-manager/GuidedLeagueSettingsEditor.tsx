...
type ParticipationMode = "flex" | "set";
...
function formFromDetail(detail: AdminLeagueManagerDetailResponse): FormState {
  ...
  return {
    ...
    participationMode: team ? "set" : optionValue(operation.participation_mode, ["flex", "set"] as const, "set"),
...
function buildDraftPatch(form: FormState, detail: AdminLeagueManagerDetailResponse): SettingsPatch {
  ...
  return {
    ...
    rules_config: {
      ...
      operation: { ...operation, session_mode: form.sessionMode, participation_mode: isTeamLeague(detail) ? "set" : form.participationMode, move_up_count: leagueFormat === "ladder" ? moveUp : 0, move_down_count: leagueFormat === "ladder" ? moveDown : 0 }
    }
  };
}
...
function SetupReview({ detail, form, team }: { detail: AdminLeagueManagerDetailResponse; form: FormState; team: boolean }) {
  ...
  const rows = [
    ...
    ["Participation model", team || form.participationMode === "set" ? "Set — roster and saved positions persist" : "Flex — choose attendees and rebuild courts each session"],
