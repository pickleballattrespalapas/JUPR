...
export function GuidedLeagueSettingsEditor({ detail, saving, canWrite, onSave, onPreview }: Props) {
  ...
  return <article style={cardStyle}>
    ...
    <p style={{ color: isClosed ? "#92400e" : "#475569" }}>{isDraft ? "Structured choices here carry into scheduling, live operation, score entry, standings, and awards. League type and singles/doubles were fixed when this draft was created." : isClosed ? `This league is ${status}; its complete saved configuration is shown read-only.` : `This league is ${status}; its complete saved configuration is shown below, and only its description can be edited.`}</p>
