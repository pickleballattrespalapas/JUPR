...
function buildScheduleConfig(form: FormState, detail: AdminLeagueManagerDetailResponse): Record<string, unknown> {
  const weeks = wholeNumber(form.weeks, "Weeks", 0, 260, true);
...
export function GuidedLeagueSettingsEditor({ detail, saving, canWrite, onSave, onPreview }: Props) {
  ...
  return <article style={cardStyle}>
    ...
    <p style={{ color: isClosed ? "#92400e" : "#475569" }}>{isDraft ? "Draft leagues expose the structured Streamlit overview, schedule, courts, competition, ratings, and awards fields. Unknown compatible extension keys are preserved." : isClosed ? `This league is ${status}; settings are read-only.` : `This league is ${status}; only its description is a safe edit.`}</p>
    ...
    {isDraft ? <div style={{ display: "grid", gap: "0.75rem", marginTop: "0.75rem" }}>
