...
function InterclubHome({ clubId, accessToken }: { clubId: string; accessToken: string }) {
  ...
  return <section className={styles.page}>
    {active && api ? <InterclubSetupWizard key={active.id} api={api} club={club} accessToken={accessToken} initialSeason={active} choices={choices} onSaved={saved}
      ...
      <header className={styles.header}><div><p className={styles.eyebrow}>{club.name}</p><h1>Interclub leagues</h1><p className={styles.muted}>Respond to invitations, prepare your club’s meet rosters, or organize a season.</p></div><button className={invitations.length ? undefined : styles.primary} disabled={!loaded} onClick={() => setActive(newSeason())}>Set up a season</button></header>
      ...
      {loaded && <>
        ...
        {participating.length > 0 && <section aria-labelledby="participating-seasons-heading">
          ...
          <p className={styles.muted}>Choose available players for each upcoming meet.</p>
          <div className={styles.cards}>{participating.map(s => <article key={s.id} className={styles.card}>
            ...
            <p className={styles.muted}>{s.details.start_date} to {s.details.end_date}</p>
            <Link className={styles.button} href={seasonHref(s)}>Prepare meet rosters</Link><p><Link href={`/admin/interclub/competition?season=${encodeURIComponent(s.id)}`}>Meet schedules, score sheets & results</Link></p>
          </article>)}</div>
        </section>}
        ...
        {organizing.length > 0 && <section><h2>Seasons your club organizes</h2><p className={styles.muted}>Manage club responses and upcoming meets.</p><div className={styles.cards}>{organizing.map(s => <article key={s.id} className={styles.card}><p className={styles.eyebrow}>Organizer</p><h3>{s.details.name}</h3><p className={styles.muted}>{s.details.start_date} to {s.details.end_date}</p><Link className={styles.button} href={seasonHref(s)}>Manage season</Link><p><Link href={`/admin/interclub/competition?season=${encodeURIComponent(s.id)}`}>Run meets & championships</Link></p><p><Link href={`/admin/interclub/publication?season=${encodeURIComponent(s.id)}`}>Public schedule, results & standings</Link></p>{seasons.some(d => d.id === s.id) && <button style={{ marginTop: 10 }} onClick={() => setActive(seasons.find(d => d.id === s.id)!)}>Club account invitations</button>}</article>)}</div></section>}
