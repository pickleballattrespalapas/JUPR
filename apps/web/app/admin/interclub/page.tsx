      ...
      {loaded && <>
        ...
        {participating.length > 0 && <section aria-labelledby="participating-seasons-heading">
          ...
          <div className={styles.cards}>{participating.map(s => <article key={s.id} className={styles.card}>
            ...
            <p className={styles.muted}>{s.details.start_date} to {s.details.end_date}</p>
            <Link className={styles.button} href={seasonHref(s)}>Open league workspace</Link>{registrationMeetPlanning(s.registration) ? <p><Link href={`/admin/interclub/competition?season=${encodeURIComponent(s.id)}`}>Meet schedules, score sheets & results</Link></p> : <p className={styles.muted}>Meet planning opens after registration closes.</p>}
          </article>)}</div>
        </section>}
        ...
        {organizing.length > 0 && <section><h2>Seasons your club organizes</h2><p className={styles.muted}>Manage your player pool, participating clubs, meet lineups, and results.</p><div className={styles.cards}>{organizing.map(s => <article key={s.id} className={styles.card}><p className={styles.eyebrow}>Organizer</p><h3>{s.details.name}</h3><p className={styles.muted}>{s.details.start_date} to {s.details.end_date}</p><Link className={styles.button} href={seasonHref(s)}>Open league workspace</Link>{registrationMeetPlanning(s.registration) ? <p><Link href={`/admin/interclub/competition?season=${encodeURIComponent(s.id)}`}>Run meets & championships</Link></p> : <p className={styles.muted}>Set the season registration dates in the league workspace. Meet planning opens after registration closes.</p>}<p><Link href={`/admin/interclub/publication?season=${encodeURIComponent(s.id)}`}>Public schedule, results & standings</Link></p>{seasons.some(d => d.id === s.id) && <button style={{ marginTop: 10 }} onClick={() => setActive(seasons.find(d => d.id === s.id)!)}>Club account invitations</button>}</article>)}</div></section>}
