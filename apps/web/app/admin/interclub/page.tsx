      ...
      {loaded && <>
        ...
        {participating.length > 0 && <section aria-labelledby="participating-seasons-heading">
          ...
          <div className={styles.cards}>{participating.map(s => <article key={s.id} className={styles.card}>
            ...
            <p className={styles.muted}>{s.details.start_date} to {s.details.end_date}</p>
            <Link className={styles.button} href={seasonHref(s)}>Prepare meet rosters</Link>
          </article>)}</div>
        </section>}
        ...
        {organizing.length > 0 && <section><h2>Seasons your club organizes</h2><p className={styles.muted}>Manage club responses and upcoming meets.</p><div className={styles.cards}>{organizing.map(s => <article key={s.id} className={styles.card}><p className={styles.eyebrow}>Organizer</p><h3>{s.details.name}</h3><p className={styles.muted}>{s.details.start_date} to {s.details.end_date}</p><Link className={styles.button} href={seasonHref(s)}>Manage season</Link><p><Link href={`/admin/interclub/publication?season=${encodeURIComponent(s.id)}`}>Public schedule, results & standings</Link></p>{seasons.some(d => d.id === s.id) && <button style={{ marginTop: 10 }} onClick={() => setActive(seasons.find(d => d.id === s.id)!)}>Club account invitations</button>}</article>)}</div></section>}
