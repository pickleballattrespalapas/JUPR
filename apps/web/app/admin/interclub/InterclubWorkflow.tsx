...
export default function InterclubWorkflow({ seasonId, meetId, current, disabled = false, unavailable = [], onSelect, hints }: {
  ...
  return <nav className={styles.workflow} aria-label="League workflow">
    <ol>{steps.map((step, index) => {
      const blocked = disabled || unavailable.includes(step.id) || (step.id !== "pool" && !meetId);
      const contents = <><span className={styles.number} aria-hidden="true">{index + 1}</span><span><strong>{step.label}</strong><small>{hints?.[step.id] || step.hint}</small></span></>;
