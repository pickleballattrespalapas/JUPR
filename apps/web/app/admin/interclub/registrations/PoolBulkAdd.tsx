...
type Props = { root: string; accessToken: string; divisions: string[]; members: SeasonPool["members"]; onAdded: (result: BulkPoolResult) => void; onClose: () => void };
...
export function PoolBulkAdd({ root, accessToken, divisions, members, onAdded, onClose }: Props) {
  ...
  return <section className={styles.bulkPanel} aria-label="Add players to season pool">
    ...
    <p className={styles.muted}>You can place linked, approved players directly into a meet lineup after confirming they can play. Email invitations are optional.</p>
    ...
    <fieldset disabled={resource.busy || resource.blocked}>
      ...
      <button disabled={resource.disabled || !requestedCount} onClick={() => void review()}>{resource.busy ? "Checking players…" : `Preview ${requestedCount || ""} player${requestedCount === 1 ? "" : "s"}`}</button>
    </fieldset>
    {preview && <div className={styles.bulkPreview}>
      ...
      <div className={styles.tableScroll}><table className={styles.poolTable}><caption>Players to add</caption><thead><tr><th scope="col">Player</th><th scope="col">Profile match</th><th scope="col">Club rating</th><th scope="col">Review</th></tr></thead><tbody>
        {preview.rows.map(row => <tr key={row.index}><th scope="row">{row.name}<span className={styles.playerMeta}>{row.email || "No email"}</span></th><td>{statusLabel[row.status]}{row.status === "ambiguous" && <label className={styles.matchChoice}>Club profile for {row.name}<select value="" disabled={resource.disabled} onChange={event => { if (event.target.value) void review(draft.map((member, index) => index === row.index ? { ...member, player_id: event.target.value === "__new__" ? null : event.target.value } : member)); }}><option value="">Choose a player…</option>{row.candidates.map(candidate => <option key={candidate.id} value={String(candidate.id)}>{candidate.name} · {poolRating(candidate.rating)} · {poolGender(candidate.gender)}</option>)}<option value="__new__">None of these — add as a new signup</option></select></label>}</td><td>{poolRating(row.rating)}{row.league_rating != null && <span className={styles.playerMeta}>League {poolRating(row.league_rating)}</span>}</td><td><button disabled={resource.disabled || draft.length === 1} onClick={() => void review(draft.filter((_, index) => index !== row.index))} aria-label={`Remove ${row.name} from this batch`}>Remove</button></td></tr>)}
      </tbody></table></div>
      ...
      <button className={styles.primary} disabled={resource.disabled || !!preview.ambiguous_count || !preview.ready_count} onClick={() => void add()}>{resource.busy ? "Adding players…" : `Add ${preview.ready_count} player${preview.ready_count === 1 ? "" : "s"} to pool`}</button>
    </div>}
  </section>;
}
