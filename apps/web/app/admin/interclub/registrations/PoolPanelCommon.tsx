...
export function ShareLink({ url, label }: { url: string; label: string }) {
  ...
  return <div className={styles.share}><label>{label}<input readOnly value={url} onFocus={event => event.currentTarget.select()} /></label>
    <button type="button" onClick={async () => { try { await navigator.clipboard.writeText(url); setCopied(true); setError(""); } catch { setError("Select the link above and copy it."); } }}>{copied ? "Link copied" : "Copy link"}</button>{error && <span role="status">{error}</span>}</div>;
}
