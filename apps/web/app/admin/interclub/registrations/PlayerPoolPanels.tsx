...
type PoolProps = { root: string; accessToken: string; clubName: string; season: RegistrationSeason; refreshKey?: number; onLateRequested?: () => void };
...
function SeasonPoolPanel({ root, accessToken, clubName, season, refreshKey, onLateRequested }: PoolProps) {
  const resource = usePoolResource<SeasonPool>(`${root}/pool`, accessToken, refreshKey);
  ...
  return <section className={styles.panel} aria-label="Season player pool">
    ...
    <p>{meetPlanningOpen ? `Review ${clubName}’s season pool and request commissioner approval for late players.` : `Add players who have committed to representing ${clubName}, or share the signup page.`} Players join the season pool once and choose their availability separately for each meet.</p>
...
function MemberCard({ member, root, accessToken, disabled, canRestore, onUpdate }: { member: PoolMember; root: string; accessToken: string; disabled: boolean; canRestore: boolean; onUpdate: (member: PoolMember, playerId: string | null, status: PoolMember["status"]) => Promise<boolean> }) {
  ...
  return <article className={styles.card}>
    ...
    <p>{member.email || "No email"} · Divisions: {member.divisions.join(", ") || "None selected"}</p>{member.notes && <p className={styles.notes}>{member.notes}</p>}
