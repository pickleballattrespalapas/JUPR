    ...
    <div style={{ display: "grid", gap: "1rem" }}>
      ...
      {dashboard ? <article style={cardStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "0.75rem", alignItems: "end" }}>
          ...
          <label>Challenger<br /><SearchablePlayerSelect aria-label="Challenger" value={createDraft.challenger_id} onValueChange={playerValue => setCreateDraft((c) => ({ ...c, challenger_id: playerValue }))} style={inputStyle}><option value="">Choose</option>{createTierPlayers.map((p) => <option key={p.player_id} value={p.player_id}>{p.player_name} · rank {p.rank ?? "—"}</option>)}</SearchablePlayerSelect></label>
          <label>Defender<br /><SearchablePlayerSelect aria-label="Defender" value={createDraft.defender_id} onValueChange={playerValue => setCreateDraft((c) => ({ ...c, defender_id: playerValue }))} style={inputStyle}><option value="">Choose</option>{createTierPlayers.filter((player) => String(player.player_id) !== createDraft.challenger_id).map((p) => <option key={p.player_id} value={p.player_id}>{p.player_name} · rank {p.rank ?? "—"}</option>)}</SearchablePlayerSelect></label>
      ...
      {dashboard ? <article style={cardStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: "0.75rem", alignItems: "end" }}>
          ...
          <label>Match A · challenger partner<br /><SearchablePlayerSelect aria-label="Match A challenger partner" value={resultDraft.a_chal} onValueChange={playerValue => setResultDraft((c) => ({ ...c, a_chal: playerValue, a_def: playerValue === c.a_def ? "" : c.a_def }))} style={inputStyle}><option value="">Choose</option>{partnerPlayers.map((p) => <option key={p.player_id} value={p.player_id}>{p.player_name}</option>)}</SearchablePlayerSelect></label>
          <label>Match A · defender partner<br /><SearchablePlayerSelect aria-label="Match A defender partner" value={resultDraft.a_def} onValueChange={playerValue => setResultDraft((c) => ({ ...c, a_def: playerValue }))} style={inputStyle}><option value="">Choose</option>{partnerPlayers.filter((player) => String(player.player_id) !== resultDraft.a_chal).map((p) => <option key={p.player_id} value={p.player_id}>{p.player_name}</option>)}</SearchablePlayerSelect></label>
          ...
          <label>Winner override<br /><SearchablePlayerSelect aria-label="Winner override" value={resultDraft.winner_override} onValueChange={playerValue => setResultDraft((c) => ({ ...c, winner_override: playerValue }))} style={inputStyle}><option value="computed">Computed</option><option value="challenger">Challenger</option><option value="defender">Defender</option></SearchablePlayerSelect></label>
      ...
      {dashboard ? <article style={cardStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "0.75rem", alignItems: "end" }}>
          ...
          <label>Forfeited by<br /><SearchablePlayerSelect aria-label="Forfeited by" value={forfeitDraft.forfeited_by_id} onValueChange={playerValue => setForfeitDraft((c) => ({ ...c, forfeited_by_id: playerValue }))} style={inputStyle}><option value="">Choose</option>{selectedForfeitChallenge?.challenger_id ? <option value={selectedForfeitChallenge.challenger_id}>{selectedForfeitChallenge.challenger_name} · challenger</option> : null}{selectedForfeitChallenge?.defender_id ? <option value={selectedForfeitChallenge.defender_id}>{selectedForfeitChallenge.defender_name} · defender</option> : null}</SearchablePlayerSelect></label>
      ...
      {dashboard ? <article style={cardStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "0.75rem", alignItems: "end" }}>
          ...
          <label>Pass used by<br /><SearchablePlayerSelect aria-label="Pass used by" value={passDraft.player_id} onValueChange={playerValue => setPassDraft((current) => ({ ...current, player_id: playerValue }))} style={inputStyle}><option value="">Choose</option>{selectedPassChallenge?.challenger_id ? <option value={selectedPassChallenge.challenger_id}>{selectedPassChallenge.challenger_name} · challenger</option> : null}{selectedPassChallenge?.defender_id ? <option value={selectedPassChallenge.defender_id}>{selectedPassChallenge.defender_name} · defender</option> : null}</SearchablePlayerSelect></label>
          <ConfirmAction
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "1rem" }}>
          <section style={{ border: "1px solid #e2e8f0", borderRadius: "12px", padding: "1rem" }}>
            ...
            <div style={{ display: "grid", gap: "0.75rem" }}>
              <label>Club player<br /><SearchablePlayerSelect aria-label="Club player" value={rosterAddDraft.player_id} onValueChange={playerValue => setRosterAddDraft((current) => ({ ...current, player_id: playerValue }))} style={inputStyle}><option value="">Choose a player not currently active</option>{addRosterOptions.map((player) => <option key={player.player_id} value={player.player_id}>{player.player_name}{inactiveRosterRows.some((row) => row.player_id === player.player_id) ? " · reactivate" : ""}</option>)}</SearchablePlayerSelect></label>
          ...
          <section style={{ border: "1px solid #e2e8f0", borderRadius: "12px", padding: "1rem" }}>
            ...
            <div style={{ display: "grid", gap: "0.75rem" }}>
              <label>Active player<br /><SearchablePlayerSelect aria-label="Active player" value={rosterMoveDraft.player_id} onValueChange={playerValue => { const player = activeRosterRows.find((row) => String(row.player_id) === playerValue); setRosterMoveDraft((current) => ({ ...current, player_id: playerValue, destination_tier: player?.tier_id === current.destination_tier ? (dashboard.tiers.find((tier) => tier.tier_id !== player.tier_id)?.tier_id || current.destination_tier) : current.destination_tier })); }} style={inputStyle}><option value="">Choose</option>{activeRosterRows.map((player) => <option key={player.player_id} value={player.player_id}>{player.player_name} · {player.tier_id} rank {player.rank ?? "—"}</option>)}</SearchablePlayerSelect></label>
      ...
      {dashboard ? <article style={cardStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: "0.75rem", alignItems: "end" }}>
          <label>Active player<br /><SearchablePlayerSelect aria-label="Active player" value={overrideDraft.player_id} onValueChange={playerValue => { const flags = dashboard.player_flags?.find((row) => String(row.player_id) === playerValue); setOverrideDraft({ player_id: playerValue, vacation_until: flags?.vacation_until || "", reinstate_required: Boolean(flags?.reinstate_required), reinstate_notes: flags?.reinstate_notes || "" }); }} style={inputStyle}><option value="">Choose</option>{activeRosterRows.map((player) => <option key={player.player_id} value={player.player_id}>{player.player_name} · {player.tier_id} rank {player.rank ?? "—"}</option>)}</SearchablePlayerSelect></label>
