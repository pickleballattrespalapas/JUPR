...
export default async function TournamentPublishPhasePage({ searchParams }: Props) {
  ...
  return (
    <section>
      <p style={{ margin: "0 0 0.5rem", color: "#2563eb", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", fontSize: "0.78rem" }}>
        Tournament Manager / Publish
      </p>
      <h1 style={{ marginTop: 0 }}>{tournamentName || "Tournament"} publish</h1>
      {error ? <p role="alert" style={{ color: "#b91c1c" }}>Tournament Publish is unavailable. {error}</p> : null}
      {data ? (
