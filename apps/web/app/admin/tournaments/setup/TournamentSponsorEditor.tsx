...
export default function TournamentSponsorEditor({ sponsors, tournamentName, disabled, onSave, onUpload }: Props) {
  ...
  async function save(rows: SponsorDraft[]) {
    if (!await onSave(rows)) throw new Error("The sponsor draft could not be saved. Please try again.");
    setMessage("Sponsor draft saved. Publish from Review when you’re ready.");
  }
  async function submit() {
    if (!draft || !draft.name.trim()) throw new Error("Enter the sponsor name.");
    if (error) throw new Error(error);
    const next = { ...draft, name: draft.name.trim(), website: normalizeSponsorWebsite(draft.website) };
    setWorking(true);
    try {
      if (file) {
        const encoded = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result).split(",")[1]); reader.onerror = () => reject(new Error("The logo could not be read.")); reader.readAsDataURL(file); });
