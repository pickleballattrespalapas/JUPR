...
export default function TournamentOpsPanel({ apiBase, clubId, status }: Props) {
  ...
  async function loadOps(tournamentId = selectedTournamentId, drawId = selectedDrawId) {
    ...
    try { await loadTournamentDetail(tournamentId); const params = new URLSearchParams(); if (drawId) params.set("draw_id", drawId); const suffix = params.toString() ? `?${params.toString()}` : ""; const payload = await requestJson<AdminTournamentOpsSnapshotResponse>(`/admin/clubs/${encodeURIComponent(clubId)}/tournaments/admin/tournaments/${encodeURIComponent(tournamentId)}/ops${suffix}`); setSnapshot(payload); resetTeamEditor(payload, drawId); resetScoreEditor(payload); setGameConfirm(""); setPlayoffConfirm(""); setMessage("Tournament operations snapshot loaded."); }
  ...
  return (
    <section style={{ display: "grid", gap: "1rem" }}>
      <article style={cardStyle}>
        ...
        <p style={{ color: "#475569" }}>Operations visibility plus guarded writes for creating draws, maintaining teams, generating/scoring round-robin games, and generating playoff games. Playoff scoring, podiums, and awards remain Streamlit-only until their write contracts are ported.</p>
