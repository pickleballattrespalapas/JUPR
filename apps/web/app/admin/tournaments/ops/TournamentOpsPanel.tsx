...
export default function TournamentOpsPanel({ apiBase, clubId, status }: Props) {
  ...
  async function awardPodium() {
    if (!selectedTournamentId || !selectedDrawId) { setMessage("Select a tournament and draw before awarding podium trophies."); return; }
    setBusy(true); setMessage(null);
    try { const payload = await requestJson<AdminTournamentWriteResponse>(`/admin/clubs/${encodeURIComponent(clubId)}/tournaments/admin/tournaments/${encodeURIComponent(selectedTournamentId)}/draws/${encodeURIComponent(selectedDrawId)}/podium/awards`, { method: "POST", body: JSON.stringify({ confirmation_text: awardConfirm, source: "next_tournament_ops_award_podium" }) }); await loadOps(selectedTournamentId, selectedDrawId); setAwardConfirm(""); setMessage(`Awarded ${payload.awarded_count ?? 0} new badge(s) from ${payload.candidate_count ?? 0} podium candidate(s).`); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Unable to award podium trophies."); }
    finally { setBusy(false); }
  }
  ...
  return (
    <section style={{ display: "grid", gap: "1rem" }}>
      <article style={cardStyle}>
        ...
        <p style={{ color: "#475569" }}>Operations visibility plus guarded writes for creating draws, importing or maintaining teams, generating/scoring round-robin games, generating/scoring playoff games, generating draw-scoped podiums, and awarding podium trophies.</p>
      ...
      {snapshot ? <>
        ...
        <article style={{ ...cardStyle, background: "#f8fafc" }}><h2 style={{ marginTop: 0 }}>Award podium trophies</h2><p style={{ color: "#475569" }}>After the draw podium has been generated and reviewed, type <code>AWARD PODIUM</code>. Badge context is draw-scoped, so multiple divisions can award their own champion, runner-up, and third-place trophies.</p><div style={{ display: "grid", gridTemplateColumns: "minmax(180px, 260px) auto", gap: "0.75rem", alignItems: "end" }}><label><strong>Type AWARD PODIUM</strong><br /><input value={awardConfirm} onChange={(event) => setAwardConfirm(event.target.value)} style={inputStyle} /></label><button type="button" onClick={awardPodium} disabled={busy || !accessToken || !selectedDrawId || awardConfirm.trim().toUpperCase() !== "AWARD PODIUM"} style={buttonStyle}>{busy ? "Awarding…" : "Award trophies"}</button></div></article>
