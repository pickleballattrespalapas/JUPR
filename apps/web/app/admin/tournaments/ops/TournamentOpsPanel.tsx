...
export default function TournamentOpsPanel({ apiBase, clubId, status }: Props) {
  ...
  async function loadOps(tournamentId = selectedTournamentId, drawId = selectedDrawId) {
    ...
    try { await loadTournamentDetail(tournamentId); const params = new URLSearchParams(); if (drawId) params.set("draw_id", drawId); const suffix = params.toString() ? `?${params.toString()}` : ""; const payload = await requestJson<AdminTournamentOpsSnapshotResponse>(`/admin/clubs/${encodeURIComponent(clubId)}/tournaments/admin/tournaments/${encodeURIComponent(tournamentId)}/ops${suffix}`); setSnapshot(payload); resetTeamEditor(payload, drawId); resetScoreEditor(payload); setGameConfirm(""); setPlayoffConfirm(""); setPodiumConfirm(""); setAwardConfirm(""); setRegistrationImportConfirm(""); setBulkTeamConfirm(""); setMessage("Tournament operations snapshot loaded."); }
  ...
  async function importRegistrations() {
    if (!selectedTournamentId || !selectedDrawId) { setMessage("Select a tournament and draw before importing registrations."); return; }
    setBusy(true); setMessage(null);
    try { const payload = await requestJson<AdminTournamentWriteResponse>(`/admin/clubs/${encodeURIComponent(clubId)}/tournaments/admin/tournaments/${encodeURIComponent(selectedTournamentId)}/draws/${encodeURIComponent(selectedDrawId)}/teams/import-registrations`, { method: "POST", body: JSON.stringify({ import_mode: registrationImportMode, confirmation_text: registrationImportConfirm, source: "next_tournament_ops_import_registrations" }) }); await loadOps(selectedTournamentId, selectedDrawId); setRegistrationImportConfirm(""); setMessage(`Imported ${payload.updated_count ?? payload.teams?.length ?? 0} registration team(s) with ${payload.import_mode || registrationImportMode} mode.`); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Unable to import registrations."); }
    finally { setBusy(false); }
  }

  async function importBulkTeams() {
    if (!selectedTournamentId || !selectedDrawId) { setMessage("Select a tournament and draw before importing teams."); return; }
    setBusy(true); setMessage(null);
    try { const payload = await requestJson<AdminTournamentWriteResponse>(`/admin/clubs/${encodeURIComponent(clubId)}/tournaments/admin/tournaments/${encodeURIComponent(selectedTournamentId)}/draws/${encodeURIComponent(selectedDrawId)}/teams/import-bulk`, { method: "POST", body: JSON.stringify({ raw_text: bulkTeamText, import_mode: bulkTeamMode, confirmation_text: bulkTeamConfirm, source: "next_tournament_ops_import_bulk_teams" }) }); await loadOps(selectedTournamentId, selectedDrawId); setBulkTeamConfirm(""); setMessage(`Imported ${payload.updated_count ?? payload.teams?.length ?? 0} bulk team(s) with ${payload.import_mode || bulkTeamMode} mode.`); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Unable to import bulk teams."); }
    finally { setBusy(false); }
  }
  ...
  async function generatePlayoffs() {
    ...
    const advanceCount = Number(playoffAdvanceCount); if (!Number.isFinite(advanceCount)) { setMessage("Advance count must be numeric."); return; }
    setBusy(true); setMessage(null);
  ...
  async function saveScore() {
    ...
    const nextA = Number(scoreA); const nextB = Number(scoreB); if (!Number.isFinite(nextA) || !Number.isFinite(nextB)) { setMessage("Both scores must be numeric."); return; }
    setBusy(true); setMessage(null);
  ...
  async function generatePodium() { if (!selectedTournamentId || !selectedDrawId) { setMessage("Select a tournament and draw before generating a podium."); return; } setBusy(true); setMessage(null); try { const payload = await requestJson<AdminTournamentWriteResponse>(`/admin/clubs/${encodeURIComponent(clubId)}/tournaments/admin/tournaments/${encodeURIComponent(selectedTournamentId)}/draws/${encodeURIComponent(selectedDrawId)}/podium`, { method: "POST", body: JSON.stringify({ confirmation_text: podiumConfirm, source: "next_tournament_ops_generate_podium" }) }); await loadOps(selectedTournamentId, selectedDrawId); setPodiumConfirm(""); setMessage(`Generated ${payload.podium?.length ?? 0} ${payload.podium_source || "draw"} podium placement(s).`); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to generate podium."); } finally { setBusy(false); } }
  async function awardPodium() { if (!selectedTournamentId || !selectedDrawId) { setMessage("Select a tournament and draw before awarding podium trophies."); return; } setBusy(true); setMessage(null); try { const payload = await requestJson<AdminTournamentWriteResponse>(`/admin/clubs/${encodeURIComponent(clubId)}/tournaments/admin/tournaments/${encodeURIComponent(selectedTournamentId)}/draws/${encodeURIComponent(selectedDrawId)}/podium/awards`, { method: "POST", body: JSON.stringify({ confirmation_text: awardConfirm, source: "next_tournament_ops_award_podium" }) }); await loadOps(selectedTournamentId, selectedDrawId); setAwardConfirm(""); setMessage(`Awarded ${payload.awarded_count ?? 0} new badge(s) from ${payload.candidate_count ?? 0} podium candidate(s).`); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to award podium trophies."); } finally { setBusy(false); } }
  ...
  return <section style={{ display: "grid", gap: "1rem" }}>
    <article style={cardStyle}><h2 style={{ marginTop: 0 }}>Tournament Ops</h2><p style={{ color: "#475569" }}>Operations visibility plus guarded writes for creating draws, importing or maintaining teams, generating/scoring games, podiums, and trophies.</p><div style={{ border: "1px solid #e2e8f0", borderRadius: "12px", padding: "0.75rem", background: accessToken ? "#f0fdf4" : "#fffbeb", marginBottom: "1rem" }}><strong>{accessToken ? `Admin session: ${adminSessionLabel(session)}` : "Admin session required"}</strong><p style={{ margin: "0.35rem 0 0", color: accessToken ? "#166534" : "#92400e" }}>{accessToken ? "Ready to load guarded tournament operations data." : sessionLoading ? "Checking admin session…" : "Sign in before loading ops data."}</p>{sessionMessage ? <p style={{ color: "#b91c1c", marginBottom: 0 }}>{sessionMessage}</p> : null}{!accessToken && !sessionLoading ? <p style={{ marginBottom: 0 }}><Link href="/admin/login">Open admin login</Link></p> : null}</div><label style={{ display: "flex", gap: "0.5rem", alignItems: "center", marginBottom: "0.75rem" }}><input type="checkbox" checked={includeArchived} onChange={(event) => setIncludeArchived(event.target.checked)} />Include archived tournaments</label><button type="button" onClick={loadTournaments} disabled={busy || !accessToken} style={buttonStyle}>{busy ? "Working…" : "Load tournaments"}</button></article>
    ...
    {snapshot ? <>
      ...
      <article style={{ ...cardStyle, background: "#f8fafc" }}><h2 style={{ marginTop: 0 }}>Import confirmed registrations</h2><p style={{ color: "#475569" }}>Imports confirmed registration entries for the selected draw's registration day/division. Each registration must already be linked to a JUPR player.</p><div style={{ display: "grid", gridTemplateColumns: "minmax(160px, 220px) minmax(200px, 260px) auto", gap: "0.75rem", alignItems: "end" }}><label><strong>Mode</strong><br /><select value={registrationImportMode} onChange={(event) => setRegistrationImportMode(event.target.value)} style={inputStyle}><option value="REPLACE">Replace current teams</option><option value="APPEND">Append after current teams</option></select></label><label><strong>Type IMPORT REGISTRATIONS</strong><br /><input value={registrationImportConfirm} onChange={(event) => setRegistrationImportConfirm(event.target.value)} style={inputStyle} /></label><button type="button" onClick={importRegistrations} disabled={busy || !accessToken || !selectedDrawId || registrationImportConfirm.trim().toUpperCase() !== "IMPORT REGISTRATIONS"} style={buttonStyle}>{busy ? "Importing…" : "Import registrations"}</button></div></article>
      <article style={{ ...cardStyle, background: "#f8fafc" }}><h2 style={{ marginTop: 0 }}>Bulk import teams</h2><p style={{ color: "#475569" }}>Paste CSV or TSV with headers like <code>Player 1, Player 2, Seed, Notes</code>. Player names or IDs must match the club roster. Import is blocked after games exist.</p><textarea value={bulkTeamText} onChange={(event) => setBulkTeamText(event.target.value)} style={{ ...inputStyle, minHeight: "8rem", fontFamily: "monospace" }} /><div style={{ display: "grid", gridTemplateColumns: "minmax(160px, 220px) minmax(180px, 260px) auto", gap: "0.75rem", alignItems: "end", marginTop: "0.75rem" }}><label><strong>Mode</strong><br /><select value={bulkTeamMode} onChange={(event) => setBulkTeamMode(event.target.value)} style={inputStyle}><option value="REPLACE">Replace current teams</option><option value="APPEND">Append after current teams</option></select></label><label><strong>Type IMPORT TEAMS</strong><br /><input value={bulkTeamConfirm} onChange={(event) => setBulkTeamConfirm(event.target.value)} style={inputStyle} /></label><button type="button" onClick={importBulkTeams} disabled={busy || !accessToken || !selectedDrawId || bulkTeamConfirm.trim().toUpperCase() !== "IMPORT TEAMS"} style={buttonStyle}>{busy ? "Importing…" : "Import teams"}</button></div></article>
      <article style={{ ...cardStyle, background: "#f8fafc" }}><h2 style={{ marginTop: 0 }}>Team editor</h2><p style={{ color: "#475569" }}>Select a DRAFT draw, assign players, then type <code>SAVE TEAMS</code>.</p><div style={{ display: "grid", gridTemplateColumns: "minmax(240px, 1fr) minmax(160px, 220px) auto", gap: "0.75rem", alignItems: "end" }}><label><strong>Draw</strong><br /><select value={selectedDrawId} onChange={(event) => { setSelectedDrawId(event.target.value); resetTeamEditor(snapshot, event.target.value); }} style={inputStyle}><option value="">Choose a draw…</option>{snapshot.draws.map((draw) => <option key={draw.id} value={draw.id}>{draw.name || draw.id}</option>)}</select></label><label><strong>Type SAVE TEAMS</strong><br /><input value={teamConfirm} onChange={(event) => setTeamConfirm(event.target.value)} style={inputStyle} /></label><button type="button" onClick={() => selectedTournamentId && selectedDrawId ? loadOps(selectedTournamentId, selectedDrawId) : undefined} disabled={!selectedTournamentId || !selectedDrawId || busy} style={ghostButtonStyle}>Reload selected draw</button></div>{selectedDrawId ? <><div style={{ overflowX: "auto", marginTop: "1rem" }}><table style={{ width: "100%", borderCollapse: "collapse", minWidth: "920px" }}><thead><tr>{["Team #", "Player 1", "Player 2", "Seed", "Notes", "Action"].map((header) => <th key={header} style={{ textAlign: "left", padding: "0.5rem", borderBottom: "1px solid #cbd5e1" }}>{header}</th>)}</tr></thead><tbody>{teamRows.map((row, index) => <tr key={index}><td style={{ padding: "0.5rem", borderBottom: "1px solid #e2e8f0" }}><input value={row.team_number} onChange={(event) => updateTeamRow(index, { team_number: event.target.value })} style={inputStyle} /></td><td style={{ padding: "0.5rem", borderBottom: "1px solid #e2e8f0" }}>{snapshot.players?.length ? <select value={playerSelectValue(row.player1_id)} onChange={(event) => updateTeamRow(index, { player1_id: event.target.value })} style={inputStyle}><option value="">Choose player…</option>{snapshot.players.map((player) => <option key={player.id} value={String(player.id)}>{player.name}</option>)}</select> : <input value={row.player1_id} onChange={(event) => updateTeamRow(index, { player1_id: event.target.value })} placeholder="player id" style={inputStyle} />}</td><td style={{ padding: "0.5rem", borderBottom: "1px solid #e2e8f0" }}>{snapshot.players?.length ? <select value={playerSelectValue(row.player2_id)} onChange={(event) => updateTeamRow(index, { player2_id: event.target.value })} style={inputStyle}><option value="">Singles / no partner</option>{snapshot.players.map((player) => <option key={player.id} value={String(player.id)}>{player.name}</option>)}</select> : <input value={row.player2_id} onChange={(event) => updateTeamRow(index, { player2_id: event.target.value })} placeholder="optional player id" style={inputStyle} />}</td><td style={{ padding: "0.5rem", borderBottom: "1px solid #e2e8f0" }}><input value={row.seed} onChange={(event) => updateTeamRow(index, { seed: event.target.value })} style={inputStyle} /></td><td style={{ padding: "0.5rem", borderBottom: "1px solid #e2e8f0" }}><input value={row.notes} onChange={(event) => updateTeamRow(index, { notes: event.target.value })} style={inputStyle} /></td><td style={{ padding: "0.5rem", borderBottom: "1px solid #e2e8f0" }}><button type="button" onClick={() => setTeamRows((current) => current.filter((_, rowIndex) => rowIndex !== index))} style={ghostButtonStyle}>Remove</button></td></tr>)}</tbody></table></div><p style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}><button type="button" onClick={() => setTeamRows((current) => [...current, { team_number: String(current.length + 1), player1_id: "", player2_id: "", seed: String(current.length + 1), notes: "" }])} style={ghostButtonStyle}>Add team row</button><button type="button" onClick={() => resetTeamEditor(snapshot, selectedDrawId)} style={ghostButtonStyle}>Reset from snapshot</button><button type="button" onClick={saveTeams} disabled={busy || !accessToken || teamConfirm.trim().toUpperCase() !== "SAVE TEAMS"} style={buttonStyle}>{busy ? "Saving…" : "Save teams"}</button></p></> : <p style={{ color: "#64748b" }}>Create or select a draw before editing teams.</p>}</article>
