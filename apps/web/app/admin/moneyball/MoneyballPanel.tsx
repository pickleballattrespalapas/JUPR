...
export default function MoneyballPanel({ apiBase, clubId, status }: Props) {
  ...
  return <div style={{ display: "grid", gap: "1rem" }}>
    ...
    <article style={cardStyle}><h2 style={{ marginTop: 0 }}>2. Select and order exactly 8 players</h2><SearchablePlayerSelect aria-label="Moneyball player selection" multiple value={selectedIds} onValuesChange={playerValues => { setSelectedIds(playerValues.slice(0, 8)); setPreview(null); setSettlement(null); }} style={inputStyle}>{players.filter((player) => player.is_active !== false).map((player) => <option key={player.id} value={player.id}>{player.name} · {(Number(player.rating || 0) / 400).toFixed(3)}</option>)}</SearchablePlayerSelect><p style={{ color: selectedCount === 8 ? "#166534" : "#92400e" }}>Selected {selectedCount}/8.</p><ol>{selectedPlayerNames.map((name, index) => <li key={selectedIds[index]} style={{ marginBottom: "0.35rem" }}><strong>P{index + 1}: {name}</strong> <button type="button" aria-label={`Move ${name} earlier`} onClick={() => moveSelected(index, -1)} disabled={index === 0} style={ghostButtonStyle}>↑</button> <button type="button" aria-label={`Move ${name} later`} onClick={() => moveSelected(index, 1)} disabled={index === selectedIds.length - 1} style={ghostButtonStyle}>↓</button></li>)}</ol><button type="button" onClick={generatePreview} disabled={busy || selectedCount !== 8} style={buttonStyle}>Generate Python schedule</button>{message ? <p role="status" aria-live="polite" style={{ color: /unable|type|uncertain|guarded/i.test(message) ? "#b91c1c" : "#166534" }}>{message}</p> : null}</article>
    ...
    {settlement ? <article style={cardStyle}><h2 style={{ marginTop: 0 }}>4. Settlement and official publish</h2>{settlement.settlement.tie_matches.length ? <p role="alert" style={{ color: "#92400e" }}>Tied matches are excluded: {settlement.settlement.tie_matches.join(", ")}.</p> : null}<div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><caption style={{ textAlign: "left", fontWeight: 800, marginBottom: "0.5rem" }}>Owes / receives settlement (net total {settlement.settlement.net_total.toFixed(2)})</caption><thead><tr><th scope="col" align="left">Player</th><th scope="col" align="right">Games</th><th scope="col" align="right">Net</th><th scope="col" align="left">Direction</th></tr></thead><tbody>{settlement.settlement.standings.map((row) => <tr key={row.player_id}><th scope="row" align="left">{row.player_name}</th><td align="right">{row.gp}</td><td align="right">{row.net.toFixed(2)}</td><td>{row.settlement_direction} {row.settlement_amount.toFixed(2)}</td></tr>)}</tbody></table></div><p><ConfirmAction triggerLabel="Publish reviewed official matches" title="Publish these Moneyball matches?" description={`This will publish ${settlement.would_publish_count} reviewed rated match(es) and apply the displayed settlement.`} confirmLabel="Yes, publish matches" confirmationText="SAVE MONEYBALL" tone="danger" disabled={busy || !writesEnabled} busy={busy} onConfirm={submitMoneyball} /> <button type="button" onClick={reviewSettlement} disabled={busy} style={ghostButtonStyle}>Refresh settlement</button></p></article> : null}
    {lastOperationKey ? <article style={{ ...cardStyle, background: "#fff7ed" }}><h2 style={{ marginTop: 0 }}>Recovery</h2><p>Operation <code>{lastOperationKey}</code>. A timeout is not proof of failure. Reconcile this exact operation before any new publish.</p><p><ConfirmAction triggerLabel="Reconcile stored response" title="Reconcile this Moneyball operation?" description="This inspects the durable response for the exact operation key without republishing matches." confirmLabel="Yes, reconcile operation" confirmationText="RECONCILE MONEYBALL" disabled={busy || !lastOperationKey} busy={busy} onConfirm={reconcileOperation} /></p><p><a href={lastResult?.correction?.match_log_url || lastResult?.recovery?.match_log_url || "/admin/match-log"}>Match Log correction</a> · <a href={lastResult?.correction?.replay_history_url || lastResult?.recovery?.replay_history_url || "/admin/replay-history"}>Replay History verification</a></p></article> : null}
  </div>;
}
