...
export default function PlayerEditorPanel({ apiBase, clubId, status }: Props) {
  ...
  const [newName, setNewName] = useState(""); const [newStartingJupr, setNewStartingJupr] = useState("");
  const [editName, setEditName] = useState(""); const [editRating, setEditRating] = useState(""); const [editStartingRating, setEditStartingRating] = useState(""); const [editActive, setEditActive] = useState(true);
  const [selectedLeagueRatingId, setSelectedLeagueRatingId] = useState(""); const [editLeagueRating, setEditLeagueRating] = useState(""); const [editLeagueStartingRating, setEditLeagueStartingRating] = useState(""); const [editLeagueActive, setEditLeagueActive] = useState(true);
  ...
  function seedEditForm(player: AdminPlayerEditorPlayer) { setEditName(player.name || ""); setEditRating(player.rating_jupr == null ? "" : String(player.rating_jupr)); setEditStartingRating(player.starting_jupr == null ? "" : String(player.starting_jupr)); setEditActive(player.active !== false); }
  function seedLeagueRatingForm(row: AdminPlayerEditorLeagueRating | null) { setSelectedLeagueRatingId(row ? String(row.id) : ""); setEditLeagueRating(row?.rating_jupr == null ? "" : String(row.rating_jupr)); setEditLeagueStartingRating(row?.starting_jupr == null ? "" : String(row.starting_jupr)); setEditLeagueActive(row?.is_active !== false); }
  ...
  function clearPlayerDetail() { setDetail(null); setEditName(""); setEditRating(""); setEditStartingRating(""); setEditActive(true); seedLeagueRatingForm(null); }
  ...
  async function createPlayer() {
    ...
    try {
      ...
      setNewName(""); setNewStartingJupr(""); setMessage("Player created or confirmed.");
    } catch (error) {
  ...
  return <section style={{ display: "grid", gap: "1rem" }}>
      ...
      {socialPeople.length ? (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "0.75rem", alignItems: "end" }}>
            <label><strong>Social identity</strong><br /><SearchablePlayerSelect aria-label="Social identity" value={selectedSocialId} onValueChange={playerValue => seedSocialForm(socialPeople.find((row) => row.id === playerValue) || null)} style={inputStyle}><option value="">Choose identity…</option>{socialPeople.map((row) => <option key={row.id} value={row.id}>{socialLabel(row)}</option>)}</SearchablePlayerSelect></label>
            ...
            <label><strong>Linked player</strong><br /><SearchablePlayerSelect aria-label="Linked player" value={socialLinkedPlayerId} onValueChange={playerValue => setSocialLinkedPlayerId(playerValue)} style={inputStyle}><option value="">Unlinked</option>{socialPlayers.map((player) => <option key={player.id} value={String(player.id)}>{player.name} #{player.id}{player.active === false ? " [inactive]" : ""}</option>)}</SearchablePlayerSelect></label>
          </div>
          <p>
            <ConfirmAction
    ...
    <article style={{ ...cardStyle, background: "#fff7ed", borderColor: "#fed7aa" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "0.75rem", alignItems: "end" }}>
        <label><strong>Source duplicate</strong><br /><SearchablePlayerSelect aria-label="Source duplicate" value={mergeSourceId} onValueChange={playerValue => { setMergeSourceId(playerValue); setMergePreview(null); }} style={inputStyle}><option value="">Choose source…</option>{players.map((player) => <option key={player.id} value={String(player.id)}>{playerOptionLabel(player)}</option>)}</SearchablePlayerSelect></label>
        <label><strong>Target keeper</strong><br /><SearchablePlayerSelect aria-label="Target keeper" value={mergeTargetId} onValueChange={playerValue => { setMergeTargetId(playerValue); setMergePreview(null); }} style={inputStyle}><option value="">Choose target…</option>{players.map((player) => <option key={player.id} value={String(player.id)}>{playerOptionLabel(player)}</option>)}</SearchablePlayerSelect></label>
