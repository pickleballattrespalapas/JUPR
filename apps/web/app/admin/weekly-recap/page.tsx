...
export default async function AdminWeeklyRecapPage() {
    ...
    <section>
      ...
      {status ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "0.75rem", marginBottom: "1rem" }}>
          ...
          <article style={cardStyle}><strong>Recaps</strong><br />{status.recap_count ?? "—"}</article>
          <article style={cardStyle}><strong>Published</strong><br />{status.published_count ?? "—"}</article>
          <article style={cardStyle}><strong>Gate</strong><br /><code>manage_matches</code></article>
        </div>
      ) : null}

      {status ? <WeeklyRecapAdminPanel apiBase={getAdminWeeklyRecapApiBaseUrl()} clubId={clubId} status={status} /> : null}
