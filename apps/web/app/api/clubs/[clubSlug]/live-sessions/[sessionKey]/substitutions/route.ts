...
export async function POST(request: Request, { params }: { params: { clubSlug: string; sessionKey: string } }) {
  ...
  try {
    const response = await fetch(
      `${base.replace(/\/$/, "")}/clubs/${encodeURIComponent(params.clubSlug)}/live-sessions/${encodeURIComponent(params.sessionKey)}/substitutions`,
      {
        method: "POST",
        cache: "no-store",
        headers: {
    ...
    return NextResponse.json({ detail: error instanceof Error ? error.message : "Unable to save JUPR Live substitution." }, { status: 502 });
  }
}
