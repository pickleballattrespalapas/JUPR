...
(async () => {
  ...
  cookie = "alpha";
  await assert.rejects(() => Home({}), /REDIRECT:\/clubs\/alpha/);
  const home = await Home({ searchParams: { welcome: "1" } });
  assert.ok(home);
  cookie = "deleted-club";
  assert.ok(
    await Home({}),
    "Deleted or unpublished remembered clubs return to homepage",
  );
  restored = {
    access_token: "verified",
    user: { email: "test@example.invalid" },
  };
  const Signup = load("app/create-club/page.tsx").default;
  await act(async () => {
    tree = create(React.createElement(Signup));
  });
  const inputs = tree.root.findAllByType("input");
  await act(async () => {
    inputs[0].props.onChange({ target: { value: "New Club" } });
    inputs[1].props.onChange({ target: { value: "new-club" } });
  });
  await act(async () =>
  ...
  assert.equal(network.at(-1).options.headers.Authorization, "Bearer verified");
  await act(async () =>
    pending(
      response(
        {
  ...
  assert.deepEqual(redirects, ["new-club"]);
  await act(async () => tree.unmount());
  console.log(
    "Public club sites: private drafts, preview/publish, conflicts, duplicate saves, club/account isolation, remembered club, safe content and authenticated signup passed.",
  );
})().catch((e) => {
  console.error(e);
  process.exitCode = 1;
