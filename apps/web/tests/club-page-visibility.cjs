...
(async () => {
  ...
  const Layout = load("app/layout.tsx").default;
  site.published = structuredClone(doc);
  const footer = renderToStaticMarkup(await Layout({ children: null }));
  assert.doesNotMatch(footer, /href="\/clubs\/tres-palapas\/(players|matches|tournaments|leagues)"/, "Global footer respects private sections");
  assert.doesNotMatch(footer, /href="\/(clubs|create-club)"/, "No directory or signup is introduced");
  ...
  assert.ok(sitemap.some(r => r.url.endsWith("/clubs/tres-palapas/leaderboards")));
  assert.ok(sitemap.some(r => r.url.endsWith("/clubs/tres-palapas/pages/visiting")));
  assert.ok(!sitemap.some(r => /\/tres-palapas\/(players|matches|tournaments|pages\/members)$/.test(r.url)));
