...
export function signupPlacement(entry: MeetSignupEntry): string {
  ...
  return entry.placement === "confirmed" ? "Spot reserved" : entry.placement === "review" ? "Waiting for profile review" : `Substitute #${entry.queue_position ?? "—"}${entry.priority === "play_up" ? " · Playing up" : ""}`;
}
