...
test("League Awards recovers and advances every persisted step without a browser-side database write", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Load leagues" }).click();
  ...
  await page.getByPlaceholder("FREEZE LEAGUE AWARDS").fill("FREEZE LEAGUE AWARDS");
  ...
  await page.getByPlaceholder("MINT AWARDS").fill("MINT AWARDS");
  ...
  await page.getByPlaceholder("ARCHIVE LEAGUE").fill("ARCHIVE LEAGUE");
