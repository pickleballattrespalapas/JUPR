...
def _sources(db: Any, *, club_id: str, assignments: list[dict[str, Any]], now: datetime,
    ...
    if administrator:
        ...
        add("staff_acceptances", "Staff invitation acceptances", f"Accepted staff invitations from the past {history_days} days.",
            "/admin/staff", "activity", "club_staff_invitations", "id,role,accepted_at",
            lambda row: _item(row, title="Staff invitation accepted", description=(
                "An operator accepted access to your club." if row.get("role") == "operator"
                else "An administrator accepted access to your club."), href="/admin/staff", time_field="accepted_at"),
            time_field="accepted_at", filters=lambda q: q.eq("status", "accepted"))
    return sources
