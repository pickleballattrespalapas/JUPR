...
def badge_management_writes_enabled(club_id: str) -> bool:
    return production_feature_enabled("badges", club_id) or (
        os.getenv("JUPR_ENV", "").lower() == "staging" and staging_write_wave_allows("badge-diagnostics")
    )


def _require_badge_write(supabase, *, club_id, workflow, required_tables):
    if not production_feature_enabled("badges", club_id):
        return require_staging_service_role_write(supabase, workflow=workflow, required_tables=required_tables)
    for table in required_tables:
        try:
            supabase.table(table).select("*").limit(1).execute()
        except Exception as exc:
            raise RuntimeError("Badge management schema is not ready. No data was changed.") from exc
...
def badge_management_options(supabase: Any, club_id: str) -> dict:
    ...
    return {"ok": True, "program": program, "players": sorted(players, key=lambda row: str(row.get("name", "")).casefold()), "seasons": seasons,
            ...
            "write_enabled": badge_management_writes_enabled(club_id)}
...
def save_badge_management(
    ...
    if not badge_management_writes_enabled(club_id):
        raise PermissionError("Badge changes are currently paused.")
    _require_badge_write(supabase, club_id=club_id, workflow="badge_management", required_tables=("badge_seasons", "admin_badge_operations"))
    ...
    if action == "award_community":
        ...
        _require_badge_write(supabase, club_id=club_id, workflow="round_robin_winner", required_tables=("badge_round_robin_decisions", "badge_program_state"))
