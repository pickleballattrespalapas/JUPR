...
def reconcile_program_badges(supabase: Any, club_id: str, *, dry_run: bool = False) -> dict:
    ...
    if dry_run:
        return preview
    if not program_badge_writes_enabled(club_id):
...
def program_badge_writes_enabled(club_id: str = "tres_palapas") -> bool:
...
def process_pending_program_badges(supabase: Any, *, limit: int = 5) -> dict:
    ...
    if production_feature_enabled("badges"):
        # Select this club before limiting, so deferred clubs cannot occupy the queue.
        rows = supabase.table("badge_program_state").select("club_id,revision,applied_revision,retry_after").eq("club_id", "tres_palapas").execute().data
        if isinstance(rows, list):
            rows = [row for row in rows if row["revision"] > row["applied_revision"]
                    and datetime.fromisoformat(row["retry_after"].replace("Z", "+00:00")) <= datetime.now(timezone.utc)]
    else:
    ...
    for row in rows:
        ...
        if not program_badge_writes_enabled(club):
            continue
        try:
